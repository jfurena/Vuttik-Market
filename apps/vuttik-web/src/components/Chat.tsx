...
export default function Chat({ currentUserId: propUserId, conversationId: propConvId, onBack }: ChatProps = {}) {
  ...
  const loadConversations = useCallback(async (page = 1) => {
    if (!currentUser) return;
    try {
      const response = await api.getConversations(currentUser.uid, page, 5);
      const convs = Array.isArray(response) ? response : (response.data || []);
      setHasMoreConvs(response.hasMore || false);
      ...
      if (page === 1) {
        ...
        if (targetConvId) {
          const exists = convs.find((c: any) => c.id === targetConvId);
          if (!exists) {
        ...
        if (currentUser.role && ['guardian', 'mega_guardian', 'admin'].includes(currentUser.role)) {
          const guardianChatExists = convs.find((c: any) => c.id === 'guardian_global_chat');
        ...
        setConversations(prev => {
           ...
           const newConvs = convs.filter((c: any) => !existingIds.has(c.id));
  ...
  const loadMessages = useCallback(async (page = 1, isPolling = false) => {
    if (!selectedConvId) return;
    if (!isPolling) {
      if (page === 1) setLoadingMsgs(true);
      else setLoadingOlderMsgs(true);
    }
    try {
      const response = await api.getMessages(selectedConvId, page, 30);
      const newMsgs = Array.isArray(response) ? response : (response.data || []);
      setHasMoreMsgs(response.hasMore || false);

      if (page === 1) {
        if (isPolling) {
          setMessages(prev => {
            const existingIds = new Set(prev.map(m => m.id));
            const addedMsgs = newMsgs.filter((m: any) => !existingIds.has(m.id));
            if (addedMsgs.length === 0) return prev;
            return [...prev, ...addedMsgs];
          });
        } else {
          setMessages(newMsgs);
        }
        ...
        setMessages(prev => {
          const existingIds = new Set(prev.map(m => m.id));
          const olderMsgs = newMsgs.filter((m: any) => !existingIds.has(m.id));
  ...
  useEffect(() => { loadConversations(convPage); }, [loadConversations, convPage]);

  useEffect(() => {
    if (selectedConvId) {
      setMsgPage(1);
      loadMessages(1, false);
      const interval = setInterval(() => loadMessages(1, true), 5000); // Poll every 5s for page 1
      return () => clearInterval(interval);
    }
  }, [selectedConvId, loadMessages]);

  useEffect(() => {
    if (selectedConvId && msgPage > 1) {
      loadMessages(msgPage, false);
    }
  }, [msgPage, selectedConvId, loadMessages]);

  // Auto-scroll to bottom when new messages arrive (only on page 1)
  useEffect(() => {
    if (msgPage === 1) {
  ...
  const handleScrollMessages = (e: React.UIEvent<HTMLDivElement>) => {
    if (e.currentTarget.scrollTop === 0 && hasMoreMsgs && !loadingOlderMsgs) {
       // Save current scroll height to restore scroll position after prepending
       const scrollHeight = e.currentTarget.scrollHeight;
       setMsgPage(prev => prev + 1);
       // We can adjust scroll position in a layout effect or after render if needed,
       // but typically prepending elements maintains the relative scroll position in modern browsers
          ...
          <div className="flex-1 overflow-y-auto no-scrollbar">
            ...
            {hasMoreConvs && (
              <button 
                ...
                className="w-full py-4 text-xs font-bold text-vuttik-blue hover:bg-vuttik-gray transition-colors border-b border-gray-50 flex items-center justify-center gap-2 disabled:opacity-50"
              >
                {loadingConvs && convPage > 1 ? (
          ...
          {selectedConv ? (
            <>
              ...
              >
                ...
                {!loadingMsgs && !loadingOlderMsgs && messages.length === 0 && (

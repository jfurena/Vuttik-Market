...
import { 
  ...
  Check, X, ShieldCheck,
  MessageSquare, User, Eye, Trash2, Ban, ShieldAlert, Activity, GitPullRequest, ThumbsUp, ThumbsDown, MapPin
} from 'lucide-react';
import { api } from '../lib/api';
import { useAuth } from '../contexts/AuthContext';
import { useDialog } from '../contexts/DialogContext';
import { useNavigate } from 'react-router-dom';
import MapDatabaseUpload from './MapDatabaseUpload';
import MapDatabaseExplorer from './MapDatabaseExplorer';
interface Report {
  id: string;
  reporter_id: string;
  target_id: string;
...
export default function GuardianDashboard({ onViewProduct }: { onViewProduct?: (id: string) => void }) {
  ...
  const [activeTab, setActiveTab] = useState<'flagged' | 'audit' | 'proposals' | 'map-db'>('flagged');
    ...
    try {
      if (action === 'dismiss') {
        ...
        const confirmed = await dialog.confirm({
          ...
          message: `Vas a eliminar "${report.target_title}". Esta acción es irreversible.`,
        ...
        const reason = await dialog.prompt({
          title: `Banear a ${report.author_name}`,
          message: 'Indica el motivo del baneo permanente.',
          placeholder: 'Motivo del baneo...'
        });
        if (reason === null) return;
        const confirmed = await dialog.confirm({
          title: `¿Banear a ${report.author_name}?`,
          message: 'Esta acción es irreversible. El usuario quedará bloqueado permanentemente.',
  ...
  const handleReporterAction = async (report: Report, action: 'strike' | 'ban') => {
    ...
    try {
      if (action === 'strike') {
        const confirmed = await dialog.confirm({
          title: `¿Dar strike a @${report.reporter_username || 'usuario'}?`,
          message: 'El usuario será penalizado por abuso del sistema de reportes.',
          confirmText: 'Sí, dar strike',
          iconType: 'warning'
        });
        if (!confirmed) return;
        ...
        const reason = await dialog.prompt({
          title: `Banear a @${report.reporter_username || 'usuario'}`,
          message: 'Indica el motivo del baneo permanente.',
          placeholder: 'Motivo del baneo...'
        });
        if (reason === null) return;
  ...
  return (
    <div className="flex flex-col gap-8 pb-32 px-4 md:px-0 max-w-full overflow-hidden">
      <AnimatePresence>
        {alertMessage && (
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        ...
        <div className="flex bg-vuttik-gray p-1 rounded-2xl overflow-x-auto no-scrollbar w-full max-w-[90vw] md:max-w-none">
          {[
            ...
            { id: 'map-db', label: 'BD Mapa', icon: MapPin },
            ...
            {activeTab === 'flagged' && (
              <div className="overflow-x-auto w-full">
                <table className="w-full text-left block md:table">
                  ...
                  <tbody className="block md:table-row-group divide-y divide-gray-50 md:divide-none">
                    {reports.filter(r => r.status === 'pending').map((report) => (
                      <tr key={report.id} className="hover:bg-red-50/30 transition-colors">
                        <td className="block md:table-cell px-2 py-3 md:px-8 md:py-6">
                          <div>
                ...
                <table className="w-full text-left block md:table">
                  ...
                  <tbody className="block md:table-row-group divide-y divide-gray-50 md:divide-none">
                    {filteredLogs.map((log) => (
                      <tr key={log.id} className="block md:table-row hover:bg-vuttik-gray/50 transition-colors bg-white p-4 mb-4 rounded-xl border border-gray-100 md:border-none md:p-0 md:mb-0 md:bg-transparent">
                        <td className="block md:table-cell px-2 py-3 md:px-8 md:py-6 whitespace-nowrap">
                          <span className="text-xs font-bold text-vuttik-navy">{new Date(log.timestamp).toLocaleString()}</span>
                        </td>
                        <td className="block md:table-cell px-2 py-3 md:px-8 md:py-6">
                          <div className="flex items-center gap-2">
                            {log.photo_url ? (
                              <img src={log.photo_url} className="w-8 h-8 rounded-full object-cover bg-gray-200" alt={log.display_name || 'U'} />
                            ) : (
                              <div className="w-8 h-8 bg-vuttik-blue/10 rounded-full flex items-center justify-center text-[10px] font-bold text-vuttik-blue">
                                {(log.display_name || 'U').charAt(0).toUpperCase()}
                              </div>
                            )}
                            <span className="text-xs font-bold text-vuttik-navy">{log.display_name || 'Desconocido'}</span>
                          </div>
                        </td>
                        <td className="block md:table-cell px-2 py-3 md:px-8 md:py-6">
                ...
                <table className="w-full text-left block md:table">
                  ...
                  <tbody className="block md:table-row-group divide-y divide-gray-50 md:divide-none">
                    {proposals.map((proposal) => (
                      <tr key={proposal.id} className="block md:table-row hover:bg-vuttik-gray/50 transition-colors bg-white p-4 mb-4 rounded-xl border border-gray-100 md:border-none md:p-0 md:mb-0 md:bg-transparent">
                        <td className="block md:table-cell px-2 py-3 md:px-8 md:py-6">
                        ...
                        <td className="block md:table-cell px-2 py-3 md:px-8 md:py-6 text-right">

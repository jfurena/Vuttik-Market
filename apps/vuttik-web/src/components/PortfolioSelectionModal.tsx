...
import { X, Plus, Folder, Lock, Globe, Check } from 'lucide-react';
...
export default function PortfolioSelectionModal({ isOpen, onClose, userId, product }: PortfolioSelectionModalProps) {
  ...
  const handleAddToPortfolio = async (portfolioId: string, isSaved: boolean) => {
    setSaving(true);
    try {
      if (isSaved) {
        await api.removeProductFromPortfolio(portfolioId, product.id, userId);
      } else {
        await api.addProductToPortfolio(portfolioId, product, 1, userId);
      }
      await fetchPortfolios(); // Refresh list to show checkmark without closing
    } catch (error) {
          ...
          <div className="p-6">
            {loading ? (
              ...
              <div className="space-y-4">
                {portfolios.length > 0 ? (
                  <div className="space-y-2 max-h-60 overflow-y-auto custom-scrollbar pr-2">
                    {portfolios.map(p => (
                      <button
                        key={p.id}
                        onClick={() => handleAddToPortfolio(p.id, (p.products || []).some((item: any) => (item.product && item.product.id === product.id) || item.id === product.id))}
                        disabled={saving}
                      ...
                      >
                        ...
                        <div className={`w-6 h-6 rounded-full border-2 flex items-center justify-center transition-colors ${(p.products || []).some((item: any) => (item.product && item.product.id === product.id) || item.id === product.id) ? 'bg-vuttik-blue border-vuttik-blue' : 'border-gray-200 group-hover:border-vuttik-blue'}`}>
                          {(p.products || []).some((item: any) => (item.product && item.product.id === product.id) || item.id === product.id) && <Check size={14} className="text-white" />}
                        </div>
                      </button>
                    ))}
                  </div>
                ...
                {saveError && <p className="text-red-500 text-sm text-center mb-2">{saveError}</p>}
                {!isCreating ? (
                  <button 
                  ...
                  <div className="p-4 bg-gray-50 border border-gray-100 rounded-2xl space-y-4 mt-4">
                    ...
                    <div className="flex gap-2 pt-2">
                      <button onClick={() => { setIsCreating(false); setIsPublic(false); setNewPortfolioName(''); }} className="flex-1 py-2 text-sm text-gray-500 font-bold">Cancelar</button>

...
import { Search, Filter, SlidersHorizontal, User as UserIcon, Globe, DollarSign, Tag, ChevronDown, Bell, MapPin, Building2, ArrowLeft, Map as MapIcon, LayoutGrid, List, X, Star, Percent, ShieldCheck } from 'lucide-react';
...
export default function P2PBoard({ 
  ...
  const [isOfficialSellerFilter, setIsOfficialSellerFilter] = useState(false);
  ...
  useEffect(() => {
    ...
    setIsOfficialSellerFilter(false);
  ...
  useEffect(() => {
    if (categories.length > 0) {
      ...
      if (currentCat && currentCat.allowedTypes && currentCat.allowedTypes.length > 0 && !currentCat.allowedTypes.includes(p2pType)) {
  ...
  const filteredProducts = products.filter(p => {
    ...
    if (isOfficialSellerFilter) {
      if (typeId !== 'sell' && !(p.isOwner || p.is_owner)) {
        matchesType = false;
      }
    }
    ...
    <div className="flex flex-col gap-3 md:gap-8 pb-32">
      ...
      <div className="w-full pl-4 sm:pl-6 md:pl-8 py-1">
        <div className="flex overflow-x-auto no-scrollbar items-center gap-2 md:gap-4 pb-2 pr-4 md:pr-8">
            {allCategories.filter(c => pinnedCategories.includes(c.id) || activeCategory === c.id).map((cat) => (
              <button
                key={cat.id}
      ...
      {showFilters && (
        ...
        >
          <div className="bg-vuttik-gray rounded-[32px] p-6 grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            ...
            <div className="space-y-2">
              <label className="text-[10px] font-bold text-vuttik-text-muted uppercase tracking-wider">Acción</label>
              <div className="relative">
            ...
            <div className="flex items-center gap-4 pt-4">
              <button 
                ...
                className={`flex-1 py-3 px-4 rounded-xl font-black text-xs sm:text-sm tracking-wide transition-all shadow-sm flex items-center justify-center gap-2 ${
                  isOfferFilter ? 'bg-red-500 text-white' : 'bg-white text-vuttik-navy border border-gray-100'
                }`}
              >
                <Percent size={16} /> Sólo Ofertas
              </button>
              
              {p2pType === 'buy' || p2pType === 'sell' ? (
                <button 
                  onClick={() => setIsOfficialSellerFilter(!isOfficialSellerFilter)}
                  className={`flex-1 py-3 px-4 rounded-xl font-black text-xs sm:text-sm tracking-wide transition-all shadow-sm flex items-center justify-center gap-2 ${
                    isOfficialSellerFilter ? 'bg-vuttik-navy text-white' : 'bg-white text-vuttik-navy border border-gray-100'
                  }`}
                >
                  <ShieldCheck size={16} /> Vendedor Oficial
                </button>
              ) : null}
              <button
                  onClick={() => {
                  ...
                  className="flex-1 items-center justify-center gap-2 px-6 py-3 rounded-xl font-bold text-sm transition-all bg-gray-100 text-gray-600 hover:bg-gray-200"
                >
                  Limpiar Filtros
              </button>
            </div>
      ...
      {/* Products Grid */}
      <div className={`px-4 sm:px-6 md:px-8 mt-4 ${viewMode === 'grid' ? 'grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4 md:gap-6' : 'flex flex-col gap-1.5'}`}>

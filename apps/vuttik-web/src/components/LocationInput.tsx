...
import { MapPin, Search, X, Loader2, ExternalLink, Map as MapIcon, Check, Store } from 'lucide-react';
...
export default function LocationInput({ value, onChange, onCoordinatesChange, placeholder, label }: LocationInputProps) {
  ...
  const searchLocations = async (text: string) => {
    ...
    setIsLoading(true);
    try {
      // 1. Search local Vuttik Places
      let localSuggestions: any[] = [];
      try {
        const localRes = await fetch(`/api/places/search?q=${encodeURIComponent(text)}`);
        if (localRes.ok) {
          const localData = await localRes.json();
          if (Array.isArray(localData)) {
            localSuggestions = localData.map((p: any) => ({
              display_name: p.direccion && p.direccion !== 'nan' ? `${p.nombre}, ${p.direccion}` : p.nombre,
              lat: p.latitud.toString(),
              lon: p.longitud.toString(),
              isVuttikPlace: true,
              category: p.categoria !== 'nan' ? p.categoria : '',
              address: {
      ...
      // 2. Limit search to a ~400x400km bounding box around the current map center
      const minLon = mapCenter[1] - 2;
      const minLat = mapCenter[0] - 2;
      const maxLon = mapCenter[1] + 2;
      const maxLat = mapCenter[0] + 2;
      const bbox = `${minLon},${minLat},${maxLon},${maxLat}`;

      const response = await fetch(
        `https://photon.komoot.io/api/?q=${encodeURIComponent(text)}&limit=8&lat=${mapCenter[0]}&lon=${mapCenter[1]}&bbox=${bbox}`
      );
      const data = await response.json();
      
      let externalSuggestions: any[] = [];
      if (data && data.features) {
        externalSuggestions = data.features.map((f: any) => {
          ...
          return {
            ...
            address: {
              country: p.country,
              state: p.state || p.city || p.county
            }
          };
        });
      }
      
      // Combine local Vuttik places with Photon map places
                    ...
                    >
                      {isLoading ? (
                        ...
                        suggestions.map((s, idx) => (
                          ...
                          >
                            <div className={`p-3 rounded-full shrink-0 ${s.isVuttikPlace ? 'bg-orange-100 text-orange-500' : 'bg-vuttik-blue/10 text-vuttik-blue'}`}>
                              {s.isVuttikPlace ? <Store size={22} /> : <MapPin size={22} />}
                            </div>

...
import { Store, User, X, Loader2, ArrowRight, Search } from 'lucide-react';
...
export default function GlobalBusinessSelector() {
  ...
  useEffect(() => {
    ...
    } else if (!showGlobalBusinessSelector) {
      setSearchQuery('');
    }
  ...
  const filteredBusinesses = useMemo(() => {
    ...
    return businesses.filter(b => b.name?.toLowerCase().includes(query) || b.codigo?.toLowerCase().includes(query));
  ...
  const handleCreate = () => {
    setShowGlobalBusinessSelector(false);
    navigate('/panel/negocio', { state: { createNew: true } });
  };
            ...
            <div className="mb-6 px-2">
              <div className="relative max-w-md mx-auto">
                ...
                <input
                  ...
                  className="w-full pl-12 pr-4 py-3 bg-gray-50 border border-gray-200 rounded-2xl text-gray-900 placeholder:text-gray-400 focus:outline-none focus:border-vuttik-blue focus:ring-2 focus:ring-vuttik-blue/20 transition-all text-sm font-bold"
                />
              </div>
            </div>
          )}
            ...
            <div className="flex-1 overflow-y-auto no-scrollbar grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6 p-2">
              ...
              {filteredBusinesses.map((b) => (
                ...
                >
                  ...
                  <h3 className="font-black text-vuttik-navy text-lg line-clamp-2 leading-tight">
                    {b.name || 'Sin Nombre'}
                  </h3>
                  <div className="mt-2 text-[10px] font-bold uppercase tracking-widest text-vuttik-text-muted">
                    Negocio
                  </div>
                </div>
              ))}

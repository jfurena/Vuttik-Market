...
const ProductCard = (props: ProductCardProps) => {
  ...
  // When publisher is just an informer, prefer showing the business name
  // For backward compatibility with older products, also check if type is 'inform'
  const displayLabel = ((isOwner === false || type === 'inform') && (storeName || business))
    ? (storeName || business)!
    : (authorName || 'Usuario');
  const isInformer = (isOwner === false || type === 'inform') && !!(storeName || business);
  ...
  if (viewMode === 'list') {
    return (
      <motion.div
        ...
        className="bg-white rounded-xl shadow-sm hover:shadow-md border border-gray-100 transition-all duration-200 cursor-pointer flex items-center gap-3 px-3 py-3 group"
      >
        {/* Thumbnail */}
        ...
        <div className="flex-1 min-w-0">
          <h3 className="text-sm font-bold text-gray-900 truncate leading-tight group-hover:text-vuttik-blue transition-colors">{title}</h3>
          ...
          <div className="flex items-center gap-1.5 mt-1">
            ...
            <span className="text-[10px] text-gray-500 font-semibold truncate">{displayLabel}</span>
            {isInformer && <Info size={9} className="text-gray-400 flex-shrink-0" title="Informado por usuario" />}
            <div className="flex items-center gap-0.5 ml-0.5">
              {[1,2,3,4,5].map(i => (
                <Star key={i} size={9} className={i <= Math.round(authorRating) ? 'text-yellow-400 fill-yellow-400' : 'text-gray-200 fill-gray-200'} />
              ))}
            </div>
          </div>
        </div>
        ...
        {/* Type badge */}
        {typeLabel && (
          <span className={`hidden sm:inline-flex px-2 py-0.5 text-[9px] font-black uppercase tracking-wider text-white rounded-md flex-shrink-0 ${typeColors[type as keyof typeof typeColors] || 'bg-gray-800'}`}>
            {typeLabel}
          </span>
        )}
        ...
        <div className="text-right flex-shrink-0">
          <div className="text-base font-black text-vuttik-blue whitespace-nowrap">{price} <span className="text-[10px] font-bold opacity-70">{currency}</span></div>
        </div>
      </motion.div>
    );
  }
        ...
        <div className="flex items-center justify-between mt-4 pt-4 border-t border-gray-100">
          ...
          >
            ...
            <div className="flex items-center gap-1">
              <span className="text-xs font-bold text-gray-700">{displayLabel}</span>
              {isInformer && <Info size={10} className="text-gray-400 flex-shrink-0" title="Informado por usuario" />}

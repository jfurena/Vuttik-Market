...
import { 
  ...
  UserCog, Ban, Unlock, AlertCircle, ClipboardList, Database
} from 'lucide-react';
...
export default function MegaGuardianDashboard() {
  const [activeView, setActiveView] = useState<'overview' | 'categories' | 'subcategories' | 'users' | 'reports' | 'subscriptions' | 'auditoria' | 'business_requests' | 'database'>('overview');
  ...
  useEffect(() => {
    if (activeView !== 'business_requests') return;
    const loadRequests = async () => {
      setLoadingRequests(true);
      try {
        const reqs = await api.getBusinessRequests();
  ...
  const handleApproveRequest = async (id: string) => {
    try {
      await api.approveBusinessRequest(id);
      setBusinessRequests(prev => prev.map(req => req.id === id ? { ...req, status: 'approved' } : req));
      setNotification({ message: 'Solicitud aprobada', type: 'success' });
      setTimeout(() => setNotification(null), 3000);
    } catch (error: any) {
      setNotification({ message: `Error: ${error.message || 'No se pudo aprobar'}`, type: 'error' });
      setTimeout(() => setNotification(null), 3000);
    }
  };
  ...
  const handleRejectRequest = async (id: string) => {
    try {
      await api.rejectBusinessRequest(id);
      setBusinessRequests(prev => prev.map(req => req.id === id ? { ...req, status: 'rejected' } : req));
      setNotification({ message: 'Solicitud rechazada', type: 'success' });
      setTimeout(() => setNotification(null), 3000);
    } catch (error: any) {
      setNotification({ message: `Error: ${error.message || 'No se pudo rechazar'}`, type: 'error' });
      setTimeout(() => setNotification(null), 3000);
    }
  };
  ...
  const handleUpdateUserRole = async (userId: string, newRole: string) => {
    try {
      ...
      if (userToUpdate) {
        // Role changes go through the dedicated admin endpoint; saveUser only
        // ever writes the caller's own profile.
      ...
      {editingPlan && (
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 md:p-0">
          ...
          >
            ...
            <div className="flex-1 min-h-0 flex flex-col md:flex-row gap-6 md:gap-12 mt-4 overflow-y-auto pr-2 pb-4">
  ...
  const renderSubcategories = () => (
    <div className="space-y-6 px-4 md:px-0">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          <h3 className="text-xl md:text-2xl font-display font-black text-vuttik-navy">Acciones</h3>
    ...
    <div className="space-y-6 px-4 md:px-0">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-vuttik-text-muted text-xs md:text-sm">Administra las categorías y sus acciones permitidas.</p>
        </div>
        <div className="flex flex-wrap gap-2 md:gap-3">
          {categories.length === 0 && (
            <button 
        ...
        <AnimatePresence>
          {(isAdding || isEditing) && (
            ...
            >
              ...
              <div className="space-y-4">
                <label className="text-[10px] font-bold text-vuttik-text-muted uppercase tracking-widest">Acciones Permitidas</label>
      ...
      <div className="bg-white border border-gray-100 rounded-[32px] md:rounded-[40px] shadow-sm p-4 md:p-8">
        <div className="flex items-center justify-between mb-6 md:mb-8">
          <div className="flex items-center gap-3">
            <div className="bg-orange-100 text-orange-500 p-2 md:p-3 rounded-xl md:rounded-2xl">
              <Store size={24} className="md:size-6" />
            </div>
            <div>
              <h2 className="text-xl md:text-2xl font-display font-black text-vuttik-navy">Peticiones POS</h2>
              <p className="text-xs md:text-sm text-vuttik-text-muted font-medium mt-1">Nuevos comercios pendientes de aprobación</p>
            </div>
          </div>
          <button 
            onClick={() => setActiveView('overview')}
            className="w-10 h-10 md:w-12 md:h-12 bg-gray-50 text-vuttik-navy rounded-full flex items-center justify-center hover:bg-gray-100 transition-all shadow-sm"
          >
        ...
        {loadingRequests ? (
          <div className="py-20 text-center">
            <div className="w-12 h-12 md:w-16 md:h-16 border-4 border-vuttik-blue border-t-transparent rounded-full animate-spin mx-auto mb-4"></div>
            <p className="text-sm md:text-base font-bold text-vuttik-text-muted uppercase tracking-widest">Cargando Peticiones...</p>
          </div>
        ) : (
          <div className="space-y-4">
            {businessRequests.map(req => (
              <div key={req.id} className="bg-gray-50 border border-gray-100 p-4 md:p-6 rounded-2xl md:rounded-3xl flex flex-col md:flex-row gap-4 justify-between items-start md:items-center">
                <div className="flex gap-4 items-start w-full md:w-auto">
                  {req.logo ? (
                    <img src={req.logo} alt="Logo" className="w-16 h-16 rounded-xl object-cover border border-gray-200" />
                  ) : (
                  ...
                  <div className="flex flex-col gap-1">
                    <h3 className="font-black text-vuttik-navy text-xl">{req.business_name || 'Negocio sin nombre'}</h3>
                    <p className="text-sm font-bold text-gray-600 mb-1">Solicitante: {req.display_name}</p>
                    <div className="flex items-center gap-2">
                      <Mail size={14} className="text-vuttik-text-muted" />
                      <a href={`mailto:${req.email}`} className="text-vuttik-blue hover:underline text-sm font-medium">{req.email}</a>
                    </div>
                    {req.location && (
                      <div className="text-xs text-gray-500 mt-1 max-w-xs break-words">
                        <span className="font-bold">Ubicación:</span> {typeof req.location === 'string' ? req.location : JSON.stringify(req.location)}
                      </div>
                    )}
                    <div className="text-xs text-vuttik-text-muted mt-2 font-medium">
                  ...
                  {req.status === 'pending' ? (
                    <>
                      <button 
                        ...
                        className="px-4 py-2 bg-red-50 text-red-600 font-bold text-sm rounded-xl hover:bg-red-600 hover:text-white transition-all flex items-center gap-2"
                      >
                        <X size={16} />
                        Rechazar
                      </button>
                      <button 
                        ...
                        className="px-4 py-2 bg-green-50 text-green-600 font-bold text-sm rounded-xl hover:bg-green-600 hover:text-white transition-all flex items-center gap-2"
                      >
                        <Check size={16} />
                        Aprobar
                      </button>
                    ...
                    <span className="px-4 py-2 bg-green-100 text-green-700 font-bold text-sm rounded-xl flex items-center gap-2">
                      <Check size={16} /> Aprobado
                    </span>
                  ) : (
                    <span className="px-4 py-2 bg-red-100 text-red-700 font-bold text-sm rounded-xl flex items-center gap-2">
                      <X size={16} /> Rechazado
                    </span>
                  )}
                </div>
            ...
            {businessRequests.length === 0 && (
              <div className="py-20 text-center bg-white border border-gray-100 rounded-[40px]">
                ...
                <p className="text-sm font-bold text-vuttik-text-muted uppercase tracking-widest">No hay peticiones de negocios</p>
              </div>
            )}
          </div>
        )}
  ...
  return (
    <div className="flex flex-col gap-6 md:gap-8 pb-32 px-4 md:px-0">
      ...
      {activeView === 'categories' ? renderCategories() : 
       ...
       activeView === 'database' ? <DatabaseExplorer /> : (
        <>
          {/* Header */}
              ...
              <div className="bg-vuttik-navy rounded-[40px] p-8 text-white">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
                  {[
                    ...
                    { title: 'Solicitudes de Negocios', desc: 'Aprobar nuevos comercios POS', icon: Store, view: 'business_requests' },
                  ].map((tool, i) => (

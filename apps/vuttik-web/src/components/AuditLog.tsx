...
export default function AuditLog() {
  ...
  const handleBan = async (targetUid: string) => {
    ...
    const confirmed = await dialog.confirm({
      ...
      message: 'Esta acción es irreversible. El usuario será bloqueado permanentemente.',
    ...
    try {
      ...
      await dialog.alert({ title: 'Éxito', message: 'Usuario baneado correctamente', iconType: 'success' });
      fetchLogs();
    } catch (err) {
                        ...
                        <div className="flex items-center gap-3">
                          <div className="w-10 h-10 rounded-xl bg-vuttik-navy flex items-center justify-center text-white text-xs font-black overflow-hidden shadow-md">
                            {log.photo_url ? (
                              ...
                              (log.display_name || (log.user_id === 'admin' ? 'Sistema' : 'U')).charAt(0).toUpperCase()
                            )}
                          </div>
                          <div>
                            <p className="text-sm font-black text-vuttik-navy">
                              {log.display_name || (log.user_id === 'admin' ? 'Sistema' : 'Usuario desconocido')}
                            </p>
                            <p className="text-[10px] text-vuttik-text-muted font-bold truncate w-24">ID: {log.user_id?.substring(0, 8) || 'N/A'}</p>
                          </div>
                        </div>
                      </td>
                      <td className="px-8 py-5">
                        ...
                        <div className="flex flex-col gap-0.5 max-w-[200px]">
                          <div className="flex items-center gap-2">
                            <span className="text-xs font-bold text-vuttik-navy capitalize truncate">
                              {log.target_type}: {log.target_id?.substring(0, 8) || 'N/A'}
                            </span>

import { Home, Search, PlusCircle, User, Settings, LogOut, MessageSquare, Shield, Briefcase, Globe, ShieldAlert, Store, ClipboardList, CreditCard, ShoppingBag, Megaphone } from 'lucide-react';
...
export default function Sidebar({ activeTab, userRole = 'user', userPlan, onPublishClick, onMarketClick }: SidebarProps) {
  ...
  const menuItems = [
    ...
    // { id: 'myplan', path: '/mi-plan', icon: CreditCard, label: 'Mi Plan' }, // DESACTIVADO TEMPORALMENTE
  ];

  // Advertising is a business-facing tool, so it appears alongside the
  // business panel rather than in the personal menu.
  if (isBusinessModeActive) {
    menuItems.push({ id: 'ads', path: '/publicidad', icon: Megaphone, label: 'Publicidad' });
  }

...
import { JWT_SECRET, authenticateToken, requireMegaGuardian, invalidateUser, authLimiter, strictLimiter, } from './middleware.js';
...
authRouter.post('/resend-verification', strictLimiter, authenticateToken, async (req, res) => {
    try {
...
authRouter.post('/google/callback', async (req, res) => {
    ...
    try {
        ...
        let user = await get('SELECT * FROM vuttik_users WHERE email = ?', [googleEmail]);
        if (user) {
            ...
            const uid = uuidv4();
            await run('INSERT INTO vuttik_users (uid, email, display_name, photo_url, role, plan_id, created_at, oauth_provider, oauth_id, email_verified) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, 1)', [uid, googleEmail, profile.name, profile.picture, 'user', 'free', new Date().toISOString(), 'google', profile.id]);
...
authRouter.post('/facebook/callback', async (req, res) => {
    ...
    try {
        ...
        let user = await get('SELECT * FROM vuttik_users WHERE oauth_provider = ? AND oauth_id = ?', ['facebook', profile.id]);
        if (!user)
...
authRouter.get('/business-requests', authenticateToken, requireMegaGuardian, async (req, res) => {
    try {
...
authRouter.post('/business-requests/:id/approve', authenticateToken, requireMegaGuardian, async (req, res) => {
    try {
...
authRouter.post('/business-requests/:id/reject', authenticateToken, requireMegaGuardian, async (req, res) => {
    try {
        const request = await get('SELECT * FROM vuttik_business_requests WHERE id = ?', [req.params.id]);
        if (!request)
            return res.status(404).json({ error: 'Solicitud no encontrada' });
...
authRouter.post('/users/:id/enable-multi-business', authenticateToken, requireMegaGuardian, async (req, res) => {
    try {
...
authRouter.get('/users/:id/pos-businesses', authenticateToken, requireMegaGuardian, async (req, res) => {
    try {
...
authRouter.post('/pos-businesses/:id/suspend', authenticateToken, requireMegaGuardian, async (req, res) => {
    try {
        const db = getDB();
...
authRouter.delete('/pos-businesses/:id', authenticateToken, requireMegaGuardian, async (req, res) => {
    try {
        const db = getDB();
        const idx = db.businesses.findIndex((b) => b.id === req.params.id);
        if (idx === -1)
...
authRouter.post('/pos-businesses/:id/transfer', authenticateToken, requireMegaGuardian, async (req, res) => {
    try {
        const { newOwnerEmail } = req.body;
        if (!newOwnerEmail)
            return res.status(400).json({ error: 'Email requerido' });
        const newUser = await get('SELECT uid FROM vuttik_users WHERE email = ?', [normalizeEmail(String(newOwnerEmail))]);
        if (!newUser)
...
const resolveTable = (table) => Object.prototype.hasOwnProperty.call(EXPLORABLE_TABLES, table) ? EXPLORABLE_TABLES[table] : null;
authRouter.get('/mega-guardian/db/:source/:table', authenticateToken, requireMegaGuardian, async (req, res) => {
    try {
        const { source, table } = req.params;
        if (source === 'sqlite') {
            if (!resolveTable(table))
                return res.status(400).json({ error: 'Tabla no permitida' });
            const rows = await all(`SELECT * FROM ${table} LIMIT 1000`);
            ...
            const db = getDB();
            if (!db[table])
                return res.status(404).json({ error: `Table ${table} not found in json db` });
...
authRouter.put('/mega-guardian/db/:source/:table/:id', authenticateToken, requireMegaGuardian, async (req, res) => {
    try {
        ...
        if (source === 'sqlite') {
            ...
            if (!primaryKeyCol)
                return res.status(400).json({ error: 'Tabla no permitida' });
            const keys = Object.keys(updateData).filter(k => SAFE_COLUMN_RE.test(k) && !PROTECTED_COLUMNS.has(k));
            if (keys.length === 0)
            ...
            const idx = db[table].findIndex((r) => r[primaryKeyCol] === id);
            if (idx === -1)
...
authRouter.delete('/mega-guardian/db/:source/:table/:id', authenticateToken, requireMegaGuardian, async (req, res) => {
    try {
        const { source, table, id } = req.params;
        if (source === 'sqlite') {
            const primaryKeyCol = resolveTable(table);

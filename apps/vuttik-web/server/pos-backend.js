...
const requireOwnerAuth = (req, res, next) => {
    const s = req.session;
    if (!s.owner_id)
        return res.status(401).json({ error: 'No autorizado. Inicia sesi├│n primero.' });
    next();
};
const requireBizAccess = (req, res, next) => {
    ...
    if (!biz) {
        // Stale session ÔÇö the business no longer exists in the DB
        s.business_id = null;
        return res.status(401).json({ error: 'Tu sesi├│n de negocio ya no es v├ílida. Por favor selecciona el negocio nuevamente.' });
    }
    if (biz.is_suspended)
        return res.status(403).json({ error: 'Este negocio ha sido suspendido por administraci├│n.' });
    return next();
};
const requireOwnerBizAccess = (req, res, next) => {
    const s = req.session;
    if (!s.owner_id || !s.business_id)
        return res.status(403).json({ error: 'Solo el due├▒o puede realizar esta acci├│n.' });
    const db = getDB();
    const biz = db.businesses.find((b) => b.id === s.business_id);
    if (!biz) {
        s.business_id = null;
...
async function startServer() {
    ...
    app.post('/api/auth/login', authLimiter, async (req, res) => {
        ...
        try {
            ...
            if (!user.password_hash)
                return res.status(401).json({ error: 'Contrase├▒a incorrecta o cuenta de Google.' });
    ...
    app.post('/api/auth/employee-login', async (req, res) => {
        ...
        if (!biz)
            return res.status(404).json({ error: 'C├│digo de negocio incorrecto.' });
        ...
        logActivity(biz, {
            ...
            accion: 'Inicio de Sesi├│n (Empleado)',
            detalles: `El empleado ${employee.nombre} ingres├│ al sistema.`,
            modulo: 'Seguridad'
        });
        saveDB(db);
        res.json({
    ...
    app.post('/api/auth/logout', (req, res) => {
        ...
        if (s.employee_id && s.business_id) {
            ...
            if (biz) {
                const emp = biz.employees?.find((e) => e.id === s.employee_id);
                if (emp) {
                    logActivity(biz, { usuario_id: emp.id, usuario_nombre: emp.nombre, accion: 'Cierre de Sesi├│n', detalles: `El empleado ${emp.nombre} sali├│ del sistema.`, modulo: 'Seguridad' });
                    saveDB(db);
                }
            }
        }
    ...
    app.get('/api/businesses', requireOwnerAuth, (req, res) => {
        ...
        const myBizList = db.businesses
            ...
            const comprasMercancia = expenses.filter((e) => e.categoria === 'Compras de Mercanc├¡a' || e.es_compra_mercancia);
            const gastosOperativos = expenses.filter((e) => e.categoria !== 'TRANSFERENCIA' && e.categoria !== 'Compras de Mercanc├¡a' && !e.es_compra_mercancia);
        ...
        if (userBusinessesCount >= 1) {
            try {
                ...
                if (!user || !user.multi_business_approved) {
                    ...
                    if (existingReq) {
                        return res.status(403).json({ error: 'pending_evaluation', message: 'Tu petici├│n est├í siendo evaluada por el Mega Guardian.' });
                    }
                    return res.status(403).json({ error: 'needs_request', message: 'Para crear m├ís de un negocio, necesitas aprobaci├│n del Mega Guardian.' });
                }
            }
            catch (err) {
    ...
    app.put('/api/businesses/:bizId', requireOwnerAuth, (req, res) => {
        ...
        const { nombre } = req.body;
        if (!nombre || !nombre.trim())
            return res.status(400).json({ error: 'Nombre inv├ílido.' });
        const s = req.session;
        const db = getDB();
        const idx = db.businesses.findIndex((b) => b.id === bizId && b.owner_id === s.owner_id);
        if (idx === -1)
    ...
    app.post('/api/employees', requireOwnerBizAccess, async (req, res) => {
        ...
        if (!nombre || !username || !password)
            return res.status(400).json({ error: 'Nombre, usuario y contrase├▒a son obligatorios.' });
        const s = req.session;
        const db = getDB();
        const biz = getBiz(db, s.business_id);
        if (!biz.employees)
        ...
        if (locations.length >= 1) {
            ...
            if (!biz.settings.allowed_location) {
                biz.settings.allowed_location = {
                    ...
                    lng: bestPoint.lng,
                    radius_meters: 200,
                    address: "Ubicaci├│n sugerida autom├íticamente (la m├ís concurrida)"
                };
            }
        }
        saveDB(db);
    ...
    app.post('/api/products', requireBizAccess, async (req, res) => {
        ...
        try {
            ...
            const location = biz.settings?.allowed_location || 'Ubicaci├│n no especificada';
            await run(`
      ...
      `, [
        ...
        if (Number(newProduct.cantidad_disponible) > 0 && Number(newProduct.costo_compra) > 0) {
            ...
            const newExpense = {
                ...
                monto: montoTotal,
                categoria: 'Compras de Mercanc├¡a',
                fecha: new Date(),
                usuario_id: uid,
        ...
        logActivity(biz, { usuario_id: usuario_id || s.owner_id || s.employee_id, usuario_nombre: req.body.usuario_nombre || 'Sistema', accion: 'Creaci├│n de Producto', detalles: `Producto creado: ${newProduct.nombre}`, modulo: 'Inventario' });
        saveDB(db);
        res.json(newProduct);
    });
    app.put('/api/products/:id', requireBizAccess, async (req, res) => {
        ...
        if (deltaCantidad !== 0 || deltaMonto !== 0) {
            ...
            biz.inventory_movements.push({
                ...
                fecha: new Date(),
                motivo: `Ajuste por edici├│n. Motivo: ${updateData.motivo_edicion || 'Correcci├│n de error'}`
            });
            if (deltaMonto !== 0) {
                if (!biz.expenses)
                    biz.expenses = [];
                biz.expenses.push({
                    id: 'exp-adj-' + Date.now(),
                    descripcion: `Ajuste de inventario: ${oldProduct.nombre} (${updateData.motivo_edicion || 'Correcci├│n'})`,
                    monto: deltaMonto,
                    categoria: 'Compras de Mercanc├¡a',
                    fecha: new Date(),
                    usuario_id: usuario_id || s.owner_id || s.employee_id,
        ...
        logActivity(biz, { usuario_id: usuario_id || s.owner_id || s.employee_id, usuario_nombre: req.body.usuario_nombre || 'Sistema', accion: 'Edici├│n de Producto', detalles: details, modulo: 'Inventario' });
        saveDB(db);
    ...
    app.delete('/api/products/:id', requireOwnerBizAccess, async (req, res) => {
        ...
        if (index === -1)
            return res.status(404).json({ error: 'Producto no encontrado' });
        const product = biz.products[index];
        logActivity(biz, { usuario_id: usuario_id || s.owner_id, usuario_nombre: usuario_nombre || 'Due├▒o', accion: 'Eliminaci├│n de Producto', detalles: `Eliminado: ${product.nombre}`, modulo: 'Inventario' });
    ...
    app.post('/api/products/:id/restock', requireBizAccess, (req, res) => {
        ...
        biz.inventory_movements.push({
            ...
            usuario_id: usuario_id || s.owner_id || s.employee_id,
            fecha: new Date(),
            motivo: motivo || 'Compra de mercanc├¡a',
            metadata: req.body.metadata || null
        });
        // Auto-register expense for this purchase
        const newExpense = {
            id: 'exp-compra-' + Date.now(),
            descripcion: `Compra: ${product.nombre} (${cantidad} ${product.unidad_venta || 'und'} ├ù RD$${costo_unitario})`,
            monto: montoTotal,
            categoria: 'Compras de Mercanc├¡a',
            fecha: new Date(),
            usuario_id: usuario_id || s.owner_id || s.employee_id,
    ...
    app.post('/api/sales', requireBizAccess, (req, res) => {
        ...
        if (sale.turno_id) {
            ...
            if (shift && shift.fecha_apertura) {
                ...
                if (shiftDate !== serverDate) {
                    return res.status(400).json({ error: 'El turno activo pertenece a un d├¡a anterior. Por favor cierra la caja y abre una nueva.' });
                }
            }
        }
        // BIZ-001 FIX: Validate stock on the server before processing ÔÇö prevents negative inventory
        ...
        if (sale.metodo_pago === 'A Cr├®dito (Fiao)' && sale.cliente_id) {
            if (!biz.clientes)
                biz.clientes = [];
            ...
            if (creditCliente && creditCliente.limite_credito > 0) {
                ...
                if (newDebt > creditCliente.limite_credito) {
                    return res.status(400).json({
                        error: `L├¡mite de cr├®dito excedido para "${creditCliente.nombre}". Deuda actual: RD$${creditCliente.deuda_actual}, L├¡mite: RD$${creditCliente.limite_credito}, Venta: RD$${sale.total}.`
                    });
                }
            }
        }
        ...
        logActivity(biz, { usuario_id: sale.usuario_id, usuario_nombre: sale.usuario_nombre || 'Empleado', accion: 'Venta Realizada', detalles: `Venta #${sale.codigo_recibo} por ${sale.total}. M├®todo: ${sale.metodo_pago}`, modulo: 'Ventas' });
        saveDB(db);
        res.json(newSale);
    });
    app.post('/api/sales/refund/:saleId', requireBizAccess, async (req, res) => {
        ...
        const db = getDB();
        const biz = getBiz(db, s.business_id);
        // Validate using owner password ÔÇö check db.owners first, then fall back to SQLite
        let valid = false;
        ...
        sale.reembolsado_por = usuario_nombre || 'Due├▒o';
        ...
        logActivity(biz, { usuario_id: usuario_id || s.owner_id, usuario_nombre: usuario_nombre || 'Due├▒o', accion: 'Reembolso de Venta', detalles: `Venta #${sale.codigo_recibo} reembolsada. Monto: ${sale.total}. Motivo: ${motivo}`, modulo: 'Ventas' });
        if (sale.items) {
    ...
    app.post('/api/sales/cancel/:saleId', requireBizAccess, (req, res) => {
        ...
        logActivity(biz, { usuario_id: s.owner_id || s.employee_id, usuario_nombre: 'Administrador', accion: 'Cancelaci├│n de Venta', detalles: `Venta #${sale.codigo_recibo} cancelada. Monto: ${sale.total}`, modulo: 'Ventas' });
        if (sale.items) {
            sale.items.forEach((item) => { const pIndex = (biz.products || []).findIndex((p) => p.id === item.producto_id); if (pIndex !== -1) {
                biz.products[pIndex].cantidad_disponible += item.cantidad;
                biz.products[pIndex].fecha_actualizacion = new Date();
    ...
    app.post('/api/shifts/open', requireBizAccess, (req, res) => {
        ...
        logActivity(biz, {
            ...
            detalles: `El usuario ${userName || 'Sistema'} abri├│ la caja con un monto inicial de RD$${Number(montoInicial)}${diferencia_apertura !== 0 ? `. Diferencia con cierre anterior: RD$${diferencia_apertura}` : ''}.`,
            modulo: 'Caja'
        });
        // If it is the first time a shift is opened, register it as Inversi├│n Externa
        if (!lastShift && Number(montoInicial) > 0) {
            ...
            biz.activity_log.unshift({
                id: `act-${Date.now()}-inv`,
                usuario_nombre: userName,
                accion: 'Inversi├│n Externa',
                detalle: `Se registr├│ Inversi├│n Externa por RD$${Number(montoInicial)} como apertura de la primera caja.`,
                fecha: new Date().toISOString(),
                modulo: 'Finanzas'
            });
        }
    ...
    app.post('/api/shifts/close/:shiftId', requireBizAccess, (req, res) => {
        ...
        logActivity(biz, {
            ...
            detalles: `El usuario ${usuarioNombre} cerr├│ la caja. Efectivo contado: RD$${Number(montoContado)}. Diferencia: RD$${diff}${motivoDiferencia ? ` (${motivoDiferencia})` : ''}.`,
            modulo: 'Caja'
        });
        saveDB(db);
        res.json(shift);
    ...
    app.post('/api/transfers', requireBizAccess, (req, res) => {
        ...
        const userName = user ? user.nombre : 'Cajero / Due├▒o';
        if (!biz.activity_log)
            biz.activity_log = [];
        biz.activity_log.unshift({
            id: `act-${Date.now()}`,
            ...
            detalle: `Se transfiri├│ RD$${monto} de ${origen} a ${destino}`,
            fecha: new Date().toISOString()
        });
        saveDB(db);
    ...
    app.get('/api/stats', requireBizAccess, (req, res) => {
        ...
        const fiadoSales = filteredSales.filter((s) => s.metodo_pago === 'A Cr├®dito (Fiao)' && s.estado !== 'cancelada' && s.estado !== 'reembolsada');
        const cobradoSales = filteredSales.filter((s) => s.metodo_pago !== 'A Cr├®dito (Fiao)' && s.estado !== 'cancelada' && s.estado !== 'reembolsada');
        ...
        // Separate operational expenses from mercanc├¡a purchases and transfers
        ...
        // Inversi├│n externa: Dinero del bolsillo del due├▒o 
        ...
        // Estimado en Caja: Tiempo real (Monto esperado del turno activo, o del ├║ltimo turno si no hay activo)
    ...
    app.post('/api/approval-requests', requireBizAccess, (req, res) => {
        ...
        logActivity(biz, { usuario_id, usuario_nombre, accion: 'Solicitud de Ubicaci├│n', detalles: `El empleado ${usuario_nombre} solicit├│ aprobaci├│n remota desde: ${requestData.address}`, modulo: 'Seguridad' });
        saveDB(db);
    ...
    app.post('/api/approval-requests/:id/action', requireOwnerBizAccess, (req, res) => {
        ...
        logActivity(biz, { usuario_id: adminId || s.owner_id, usuario_nombre: adminNombre || 'Due├▒o', accion: action === 'aprobar' ? 'Aprobar Ubicaci├│n' : 'Rechazar Ubicaci├│n', detalles: `El due├▒o ${action === 'aprobar' ? 'aprob├│' : 'rechaz├│'} el login remoto para ${request.usuario_nombre}.`, modulo: 'Seguridad' });
        saveDB(db);
        res.json(request);
    });
    ...
    app.post('/api/clientes', requireBizAccess, (req, res) => {
        try {
            ...
            logActivity(biz, { usuario_id: s.owner_id || s.employee_id, usuario_nombre: req.body.usuario_nombre || 'Sistema', accion: 'Creaci├│n de Cliente', detalles: `Cliente de cr├®dito creado: ${nombre} (L├¡mite: RD$${limite_credito})`, modulo: 'Clientes' });
            saveDB(db);
    ...
    app.put('/api/clientes/:id', requireBizAccess, (req, res) => {
        ...
        logActivity(biz, { usuario_id: s.owner_id || s.employee_id, usuario_nombre: req.body.usuario_nombre || 'Sistema', accion: 'Edici├│n de Cliente', detalles: `Cliente ${cliente.nombre} actualizado.`, modulo: 'Clientes' });
        saveDB(db);
        res.json(cliente);
    });
    app.post('/api/clientes/:id/pay', requireBizAccess, (req, res) => {
        ...
        logActivity(biz, { usuario_id: usuario_id || s.owner_id || s.employee_id, usuario_nombre: usuario_nombre || 'Sistema', accion: 'Abono de Deuda', detalles: `Cliente ${cliente.nombre} abon├│ RD$${abono}. Motivo: ${newPago.motivo}`, modulo: 'Clientes' });
        saveDB(db);
    ...
    // SEC-001 FIX: Backup restricted to owner only ÔÇö employees cannot download db.json
    ...
    // === GLOBAL ERROR HANDLER ÔÇö must be BEFORE static/vite middleware ===
    app.use((err, req, res, next) => {
        console.error('Unhandled server error:', err);
        if (res.headersSent)
            return next(err);
        if (err?.message === 'Negocio no encontrado') {
            return res.status(401).json({ error: 'Negocio no encontrado o sesi├│n inv├ílida.' });
        }
        res.status(500).json({ error: err?.message || 'Error interno del servidor.' });
    });
    if (process.env.NODE_ENV === 'production') {
        // En producci├│n (Electron), servir la carpeta dist compilada

...
import { authenticateToken, optionalAuth, requireMegaGuardian, JWT_SECRET, } from './middleware.js';
export const adsRouter = express.Router();
/** Slots the client may request. Anything else is rejected. */
export const PLACEMENTS = ['feed', 'sidebar', 'product_detail', 'search', 'interstitial'];
...
function matchesTargeting(campaign, ctx) {
    ...
    if (countries.length && (!ctx.country || !countries.includes(ctx.country)))
        return false;
    ...
    if (categories.length && (!ctx.categoryId || !categories.includes(ctx.categoryId)))
        return false;
    return true;
}
 ...
 */
function signServe(creativeId, campaignId, issuedAt) {
    return crypto
        .createHmac('sha256', JWT_SECRET())
        .update(`${creativeId}.${campaignId}.${issuedAt}`)
        .digest('hex');
}
function makeServeToken(creativeId, campaignId) {
    const issuedAt = Date.now();
    return `${issuedAt}.${signServe(creativeId, campaignId, issuedAt)}`;
}
function verifyServeToken(token, creativeId, campaignId) {
    ...
    if (!issuedAt || !signature)
        return false;
    if (Date.now() - issuedAt > SERVE_TOKEN_TTL_MS)
        return false;
...
async function planHidesAds(planId) {
    if (!planId)
        return false;
    const plan = await get('SELECT features FROM vuttik_subscription_plans WHERE id = ?', [planId]);
    if (!plan)
        return false;
 ...
 * has nothing to show.
 */
adsRouter.get('/serve', optionalAuth, async (req, res) => {
    const placement = String(req.query.placement || 'feed');
    ...
    try {
        ...
        const rows = await all(`SELECT cr.*, c.bid_cpm, c.target_countries, c.target_provinces, c.target_categories
        ...
        WHERE cr.placement = ?
          ...
          AND (c.ends_at   IS NULL OR c.ends_at   >= ?)`, [placement, now, now]);
...
function adsenseConfig(placement) {
...
// --- Tracking --------------------------------------------------------------
async function recordEvent(type, body, req) {
    ...
    const campaign = await get('SELECT id, bid_cpm, budget_total, budget_spent, status FROM vuttik_ad_campaigns WHERE id = ?', [campaignId]);
    if (!campaign)
        return { status: 404, payload: { error: 'Campaña no encontrada' } };
    await run(`INSERT INTO vuttik_ad_events (id, campaign_id, creative_id, type, user_uid, placement, country, category_id, created_at)
     VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)`, [
    ...
    if (type === 'impression') {
        ...
        await run(`UPDATE vuttik_ad_campaigns SET budget_spent = ?, status = ?, updated_at = ? WHERE id = ?`, [spent, exhausted ? 'completed' : campaign.status, new Date().toISOString(), campaignId]);
    }
    return { status: 200, payload: { success: true } };
}
adsRouter.post('/impression', optionalAuth, async (req, res) => {
    try {
...
adsRouter.post('/click', optionalAuth, async (req, res) => {
    try {
...
adsRouter.get('/campaigns', authenticateToken, async (req, res) => {
    try {
        const campaigns = await all('SELECT * FROM vuttik_ad_campaigns WHERE advertiser_uid = ? ORDER BY created_at DESC', [req.user.uid]);
        const enriched = await Promise.all(campaigns.map(async (c) => {
            const stats = await get(`SELECT
           SUM(CASE WHEN type = 'impression' THEN 1 ELSE 0 END) as impressions,
           SUM(CASE WHEN type = 'click'      THEN 1 ELSE 0 END) as clicks
         FROM vuttik_ad_events WHERE campaign_id = ?`, [c.id]);
...
adsRouter.post('/campaigns', authenticateToken, async (req, res) => {
    ...
    try {
        const id = uuidv4();
        const now = new Date().toISOString();
        await run(`INSERT INTO vuttik_ad_campaigns
...
async function loadOwnedCampaign(req, campaignId) {
    const campaign = await get('SELECT * FROM vuttik_ad_campaigns WHERE id = ?', [campaignId]);
    if (!campaign)
        return { error: 404 };
...
adsRouter.patch('/campaigns/:id', authenticateToken, async (req, res) => {
    try {
        ...
        const current = await get('SELECT status FROM vuttik_ad_campaigns WHERE id = ?', [req.params.id]);
        if (current.status === 'pending' || current.status === 'rejected') {
            return res.status(400).json({ error: 'La campaña aún no ha sido aprobada' });
        }
        await run('UPDATE vuttik_ad_campaigns SET status = ?, updated_at = ? WHERE id = ?', [status, new Date().toISOString(), req.params.id]);
...
adsRouter.get('/campaigns/:id/creatives', authenticateToken, async (req, res) => {
    try {
        const { error } = await loadOwnedCampaign(req, req.params.id);
        if (error === 404)
            return res.status(404).json({ error: 'Campaña no encontrada' });
        if (error === 403)
            return res.status(403).json({ error: 'Acceso denegado' });
        const creatives = await all('SELECT * FROM vuttik_ad_creatives WHERE campaign_id = ? ORDER BY created_at DESC', [req.params.id]);
        res.json(creatives);
    }
    catch (error) {
        res.status(500).json({ error: error.message });
...
adsRouter.post('/campaigns/:id/creatives', authenticateToken, async (req, res) => {
    ...
    try {
        ...
        const id = uuidv4();
        await run(`INSERT INTO vuttik_ad_creatives
         (id, campaign_id, placement, headline, body, image_url, cta_label, destination_url, is_active, created_at)
       VALUES (?, ?, ?, ?, ?, ?, ?, ?, 1, ?)`, [id, req.params.id, placement, headline.trim(), body || null, imageUrl || null,
...
adsRouter.delete('/creatives/:id', authenticateToken, async (req, res) => {
    try {
        const creative = await get('SELECT campaign_id FROM vuttik_ad_creatives WHERE id = ?', [req.params.id]);
        if (!creative)
...
// --- Moderation ------------------------------------------------------------
adsRouter.get('/admin/campaigns', authenticateToken, requireMegaGuardian, async (req, res) => {
    try {
        ...
        const rows = status
            ? await all(`SELECT c.*, u.display_name as advertiser_name, u.email as advertiser_email
...
adsRouter.post('/admin/campaigns/:id/review', authenticateToken, requireMegaGuardian, async (req, res) => {
    const { decision, note } = req.body;
    if (!['approve', 'reject'].includes(decision)) {
        return res.status(400).json({ error: 'Decisión inválida' });
    }
    try {
        const campaign = await get('SELECT id FROM vuttik_ad_campaigns WHERE id = ?', [req.params.id]);
        if (!campaign)
            return res.status(404).json({ error: 'Campaña no encontrada' });
        await run('UPDATE vuttik_ad_campaigns SET status = ?, review_note = ?, updated_at = ? WHERE id = ?', [decision === 'approve' ? 'active' : 'rejected', note || null, new Date().toISOString(), req.params.id]);
        res.json({ success: true });
    }
    catch (error) {
        res.status(500).json({ error: error.message });

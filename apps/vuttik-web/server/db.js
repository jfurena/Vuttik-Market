...
const dbPath = process.env.VUTTIK_DB_PATH || (isProd ? '/var/www/vuttik/data/vuttik.db' : path.resolve(__dirname, '../vuttik.db'));
...
export async function initDB() {
    ...
    // Auto-migration to add date_of_birth to vuttik_users
    try {
        await run(`ALTER TABLE vuttik_users ADD COLUMN date_of_birth TEXT`);
        console.log('Added date_of_birth column to vuttik_users');
    }
    catch (e) {
    ...
    try {
        const dbJsonPath = process.env.VUTTIK_DB_JSON_PATH || (process.env.USER_DATA_PATH ? path.join(process.env.USER_DATA_PATH, 'db.json') : path.join(__dirname, '../data/db.json'));
    ...
    CREATE TABLE IF NOT EXISTS vuttik_users (
      ...
      show_working_hours BOOLEAN DEFAULT 1
    )
  `);
    try {
        await run("ALTER TABLE vuttik_users ADD COLUMN password_hash TEXT");
    }
    catch (e) { }
    try {
        await run("ALTER TABLE vuttik_users ADD COLUMN oauth_provider TEXT");
    }
    catch (e) { }
    try {
        await run("ALTER TABLE vuttik_users ADD COLUMN oauth_id TEXT");
    }
    catch (e) { }
    try {
        await run("ALTER TABLE vuttik_users ADD COLUMN onboarding_completed BOOLEAN DEFAULT 0");
    }
    catch (e) { }
    try {
        await run("ALTER TABLE vuttik_users ADD COLUMN age INTEGER");
    }
    catch (e) { }
    try {
        await run("ALTER TABLE vuttik_users ADD COLUMN gender TEXT");
    }
    catch (e) { }
    try {
        await run("ALTER TABLE vuttik_users ADD COLUMN country TEXT");
    }
    catch (e) { }
    try {
        await run("ALTER TABLE vuttik_users ADD COLUMN language TEXT");
    }
    catch (e) { }
    try {
        await run("ALTER TABLE vuttik_users ADD COLUMN username TEXT");
    }
    catch (e) { }
    try {
        await run("ALTER TABLE vuttik_users ADD COLUMN username_changes TEXT DEFAULT '[]'");
    }
    catch (e) { }
    try {
        await run("ALTER TABLE vuttik_users ADD COLUMN email_verified BOOLEAN DEFAULT 0");
    }
    catch (e) { }
    try {
        await run("ALTER TABLE vuttik_users ADD COLUMN verification_token TEXT");
    }
    catch (e) { }
    try {
        await run("ALTER TABLE vuttik_users ADD COLUMN next_billing_date TEXT");
    }
    catch (e) { }
    try {
        await run("ALTER TABLE vuttik_users ADD COLUMN active_profile_mode TEXT DEFAULT 'personal'");
    }
    catch (e) { }
    try {
        await run("ALTER TABLE vuttik_users ADD COLUMN is_banned BOOLEAN DEFAULT 0");
    }
    catch (e) { }
    try {
    ...
    // Auto-migration to add business_name, logo, and location to vuttik_business_requests
    try {
        await run(`ALTER TABLE vuttik_business_requests ADD COLUMN business_name TEXT`);
        await run(`ALTER TABLE vuttik_business_requests ADD COLUMN logo TEXT`);
        await run(`ALTER TABLE vuttik_business_requests ADD COLUMN location TEXT`);
        console.log('Added business_name, logo, and location to vuttik_business_requests');
    }
    catch (e) {
        // Columns might already exist
    }
    // Auto-migration to add user_name and user_email for self-contained requests
    try {
        await run(`ALTER TABLE vuttik_business_requests ADD COLUMN user_name TEXT`);
        console.log('Added user_name to vuttik_business_requests');
    } catch (e) { /* already exists */ }
    try {
        await run(`ALTER TABLE vuttik_business_requests ADD COLUMN user_email TEXT`);
        console.log('Added user_email to vuttik_business_requests');
    } catch (e) { /* already exists */ }
    ...
    try {
        await run("ALTER TABLE vuttik_users ADD COLUMN reset_password_token TEXT");
    }
    catch (e) { }
    try {
        await run("ALTER TABLE vuttik_users ADD COLUMN reset_password_expires TEXT");
    }
    catch (e) { }
    try {
        await run("ALTER TABLE vuttik_users ADD COLUMN bio TEXT");
    }
    catch (e) { }
    try {
        await run("ALTER TABLE vuttik_users ADD COLUMN location TEXT");
    }
    catch (e) { }
    try {
        await run("ALTER TABLE vuttik_users ADD COLUMN phone TEXT");
    }
    catch (e) { }
    try {
        await run("ALTER TABLE vuttik_users ADD COLUMN social_links TEXT");
    }
    catch (e) { }
    try {
        await run("ALTER TABLE vuttik_users ADD COLUMN working_hours TEXT");
    }
    catch (e) { }
    try {
        await run("ALTER TABLE vuttik_users ADD COLUMN location_privacy TEXT");
    }
    catch (e) { }
    try {
        await run("ALTER TABLE vuttik_users ADD COLUMN show_working_hours BOOLEAN DEFAULT 1");
    }
    catch (e) { }
    // Notifications Table
    await run(`
  ...
  `);
    ...
    try {
        await run("ALTER TABLE vuttik_categories ADD COLUMN icon TEXT DEFAULT 'Tag'");
    }
    catch (e) { }
    try {
        await run("ALTER TABLE vuttik_categories ADD COLUMN created_by TEXT");
    }
    catch (e) { }
    try {
        await run("ALTER TABLE vuttik_categories ADD COLUMN is_service BOOLEAN DEFAULT 0");
    }
    catch (e) { }
    try {
        await run("ALTER TABLE vuttik_categories ADD COLUMN requires_ean BOOLEAN DEFAULT 0");
    }
    catch (e) { }
  ...
  `);
    ...
    try {
        await run("ALTER TABLE vuttik_transaction_types ADD COLUMN icon TEXT DEFAULT 'Tag'");
    }
    catch (e) { }
    try {
        await run("ALTER TABLE vuttik_transaction_types ADD COLUMN active BOOLEAN DEFAULT 1");
    }
    catch (e) { }
  ...
  `);
    try {
        await run("ALTER TABLE vuttik_subscription_plans ADD COLUMN is_hidden BOOLEAN DEFAULT 0");
    }
    catch (e) { }
    try {
        await run("ALTER TABLE vuttik_subscription_plans ADD COLUMN is_coming_soon BOOLEAN DEFAULT 0");
    }
    catch (e) { }
    try {
        await run("ALTER TABLE vuttik_subscription_plans ADD COLUMN is_recommended BOOLEAN DEFAULT 0");
    }
    catch (e) { }
    try {
        await run("ALTER TABLE vuttik_subscription_plans ADD COLUMN order_index INTEGER DEFAULT 0");
    }
    catch (e) { }
    // Products Table
    await run(`
  ...
  `);
    ...
    try {
        await run("ALTER TABLE vuttik_products ADD COLUMN barcode TEXT");
    }
    catch (e) { }
    try {
        await run("ALTER TABLE vuttik_products ADD COLUMN phone TEXT");
    }
    catch (e) { }
    try {
        await run("ALTER TABLE vuttik_products ADD COLUMN lat REAL");
    }
    catch (e) { }
    try {
        await run("ALTER TABLE vuttik_products ADD COLUMN lng REAL");
    }
    catch (e) { }
    try {
        await run("ALTER TABLE vuttik_products ADD COLUMN is_on_sale BOOLEAN DEFAULT 0");
    }
    catch (e) { }
    try {
        await run("ALTER TABLE vuttik_products ADD COLUMN sale_price REAL");
    }
    catch (e) { }
    try {
        await run("ALTER TABLE vuttik_products ADD COLUMN chain TEXT");
    }
    catch (e) { }
    try {
        await run("ALTER TABLE vuttik_products ADD COLUMN store_name TEXT");
    }
    catch (e) { }
    try {
        await run("ALTER TABLE vuttik_products ADD COLUMN is_independent BOOLEAN DEFAULT 0");
    }
    catch (e) { }
    try {
        await run("ALTER TABLE vuttik_products ADD COLUMN country TEXT");
    }
    catch (e) { }
    try {
  ...
  `);
    ...
    for (const chain of defaultChains) {
        try {
            await run('INSERT INTO vuttik_chains (id, name, created_at) VALUES (?, ?, ?)', [chain.id, chain.name, new Date().toISOString()]);
        }
        catch (e) {
  ...
  `);
    ...
    try {
        await run("ALTER TABLE vuttik_products ADD COLUMN posted_as TEXT DEFAULT 'personal'");
    }
    catch (e) { }
    // Social Posts Table
    await run(`
  ...
  `);
    try {
        await run("ALTER TABLE vuttik_posts ADD COLUMN posted_as TEXT DEFAULT 'personal'");
    }
    catch (e) { }
  ...
  `);
    try {
        await run(`ALTER TABLE vuttik_post_likes ADD COLUMN created_at TEXT`);
    } catch (e) {}
    // Metrics Table
    await run(`
  ...
  `);
    try {
        await run("ALTER TABLE vuttik_product_follows ADD COLUMN entity_type TEXT DEFAULT 'product_id'");
    }
    catch (e) { }
    try {
        await run("ALTER TABLE vuttik_product_follows ADD COLUMN entity_value TEXT");
    }
    catch (e) { }
  ...
  `);
    try {
        await run("ALTER TABLE vuttik_conversations ADD COLUMN p1_name TEXT");
    }
    catch (e) { }
    try {
        await run("ALTER TABLE vuttik_conversations ADD COLUMN p1_photo TEXT");
    }
    catch (e) { }
    try {
        await run("ALTER TABLE vuttik_conversations ADD COLUMN p2_name TEXT");
    }
    catch (e) { }
    try {
        await run("ALTER TABLE vuttik_conversations ADD COLUMN p2_photo TEXT");
    }
    catch (e) { }
  ...
  `);
    try {
        await run("ALTER TABLE vuttik_notifications ADD COLUMN type TEXT");
    }
    catch (e) { }
    // EAN Database Table
    await run(`
  ...
  `);
    ...
    await run(`
    CREATE TABLE IF NOT EXISTS vuttik_ratings (
      id TEXT PRIMARY KEY,
  ...
  `);
    await run('CREATE INDEX IF NOT EXISTS idx_ratings_target ON vuttik_ratings(target_id)');
    ...
    try {
        const result = await get("SELECT COUNT(*) as count FROM vuttik_categories WHERE id = 'TECNOLOGIA'");
        ...
        if (!hasNewCategories) {
            ...
            for (const cat of initialCategories) {
                await run('INSERT OR REPLACE INTO vuttik_categories (id, name, icon, order_index, allowed_types, fields, system_fields) VALUES (?, ?, ?, ?, ?, ?, ?)', [cat.id, cat.name, 'Tag', cat.order, JSON.stringify(cat.types), JSON.stringify(cat.fields), JSON.stringify(cat.sys)]);
            }

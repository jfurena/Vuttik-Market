...
async function loadUser(uid) {
    ...
    if (cached)
        return cached;
    const row = await get('SELECT uid, email, role, plan_id, is_banned FROM vuttik_users WHERE uid = ?', [uid]);
    if (!row)
        return null;
    const user = {
        uid: row.uid,
...
export function invalidateUser(uid) {
 ...
 */
export const requireSelf = (paramName = 'uid', ...alsoAllowRoles) => (req, res, next) => {
    if (!req.user)
        return res.status(401).json({ error: 'No autenticado' });
    const target = req.params[paramName];
    if (target === req.user.uid)
        return next();
    if (alsoAllowRoles.includes(req.user.role))
        return next();
    return res.status(403).json({ error: 'Acceso denegado' });
};
/**
 ...
 */
export async function userCanActForBusiness(userUid, businessUid) {
    if (!businessUid)
        return false;
    if (businessUid === userUid)
        return true;
    const owned = await get('SELECT uid FROM vuttik_business_profiles WHERE uid = ? AND owner_uid = ?', [businessUid, userUid]);
    if (owned)
        return true;
    const member = await get("SELECT id FROM vuttik_business_members WHERE business_uid = ? AND member_uid = ? AND status = 'accepted'", [businessUid, userUid]);
    return !!member;
}
/**
 ...
 */
export const requireBusinessAccess = (paramName = 'uid') => async (req, res, next) => {
    if (!req.user)
        return res.status(401).json({ error: 'No autenticado' });
    ...
    if (req.user.role === 'mega_guardian')
        return next();
    try {
        if (await userCanActForBusiness(req.user.uid, businessUid))
            return next();
 ...
 */
export async function resolveActingUid(req, requestedUid) {
    const self = req.user.uid;
    if (!requestedUid || requestedUid === self)
        return self;
    if (req.user.role === 'mega_guardian')
        return requestedUid;
    if (await userCanActForBusiness(self, requestedUid))
        return requestedUid;
    return self;
}
// --- Rate limiters ---------------------------------------------------------

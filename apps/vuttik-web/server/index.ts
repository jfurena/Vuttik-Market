...
import { adsRouter, initAdsSchema } from './ads.js';
import {
    authenticateToken,
    optionalAuth,
    requireRole,
...
// Refuse to serve production traffic with guessable signing keys. Previously
// these silently fell back to a public default, which would let anyone mint a
// valid mega_guardian token.
const REQUIRED_PROD_SECRETS = ['JWT_SECRET', 'SESSION_SECRET'];
if (process.env.NODE_ENV === 'production') {
    const missing = REQUIRED_PROD_SECRETS.filter(k => !process.env[k] || process.env[k].length < 32);
    if (missing.length) {
        console.error(
            `FATAL: ${missing.join(', ')} must be set to a value of at least 32 characters in production.`
        );
        process.exit(1);
    }
}

// Express sits behind nginx in production; without this the rate limiters and
// request logs would all attribute traffic to the proxy's address.
...
// Broad ceiling so a single client cannot saturate the API. Applied by path
// inside a plain middleware because the mounted form does not line up with the
// Express typings shipped in this workspace.
app.use((req, res, next) => {
    if (req.path.startsWith('/api/')) return globalLimiter(req, res, next);
    next();
});
const sessionSecret = process.env.SESSION_SECRET || 'fallback-dev-secret-change-in-production';
// Cast: @types/express-session resolves a different copy of
// @types/express-serve-static-core than express itself, so the handler types
// do not line up. Runtime behaviour is unaffected.
app.use((session as any)({
    name: 'vuttik_pos_sid',
    secret: sessionSecret,
    // Only persist sessions that actually hold something: the previous
    // resave/saveUninitialized combination wrote a session for every anonymous
    // request, which grows memory without bound.
    resave: false,
...
// AI Image Scanner (Gemini 1.5 Flash)
app.post('/api/ai/scan-image', authenticateToken, aiLimiter, async (req, res) => {
  const { imageBase64, mode } = req.body; // mode: 'menu' | 'product'
  ...
  try {
    ...
    let promptText = '';
    if (mode === 'menu') {
      promptText = 'Extract all food/beverage products and their prices from this menu image. Return a JSON array. Each object must have: "title" (string, the product name), "price" (number, the extracted price without currency symbols), and "category" (string, short category name like "Bebidas", "Entradas", "Platos Principales"). Return ONLY the raw JSON array (do not wrap it in markdown block like ```json).';
    } else if (mode === 'receipt') {
      promptText = 'Extract all products and their prices from this receipt or invoice image. Return a JSON array. Each object must have: "title" (string, the product name), "price" (number, the extracted price without currency symbols), and "category" (string, a short category name guessed based on the product, like "Alimentos", "Limpieza", "Hogar", etc). Return ONLY the raw JSON array (do not wrap it in markdown block).';
    } else {
      promptText = 'Extract the single main product and its price from this image (e.g. from a store shelf label). Return a JSON object. It must have: "title" (string, the product name), "price" (number, the regular price without currency symbols), "discountPrice" (number, if there is a visible sale/discount price, otherwise null), and "description" (string, a short generated description based on the product packaging). Return ONLY the raw JSON object (do not wrap it in markdown block).';
    }
    ...
    if (!text) {
      return res.status(500).json({ error: 'No data extracted from image' });
    }
...
/** True when the user is one of the two participants of the conversation. */
async function isConversationParticipant(conversationId, userUid) {
    if (!conversationId || !userUid) return false;
    const row = await get('SELECT participant_1, participant_2 FROM vuttik_conversations WHERE id = ?', [conversationId]);
    if (!row) return false;
    return row.participant_1 === userUid || row.participant_2 === userUid;
}

/**
 * True when the signed-in user may edit content attributed to `ownerUid`.
 * Business content is editable by anyone with access to that business.
 */
async function canEditContent(req, ownerUid) {
    if (!ownerUid) return false;
    if (ownerUid === req.user.uid) return true;
    if (req.user.role === 'mega_guardian') return true;
    return userCanActForBusiness(req.user.uid, ownerUid);
}

/** Fields that must never reach a client, even the account's own owner. */
const SENSITIVE_USER_FIELDS = ['password_hash', 'reset_password_token', 'reset_password_expires', 'verification_token'];

/** Strips credential material from a user row before serialising it. */
...
app.post('/api/metrics', optionalAuth, async (req, res) => {
...
app.get('/api/users/:uid/analytics', authenticateToken, requireSelf('uid', 'mega_guardian'), async (req, res) => {
...
app.get('/api/admin/audit-log', authenticateToken, requireMegaGuardian, async (req, res) => {
    const NOISE_EVENTS = [
...
app.get('/api/users/suggest-username', async (req, res) => {
    ...
    try {
        ...
        if (!baseName)
            return res.json({ suggestion: 'usuario' + Math.floor(Math.random() * 10000) });
...
app.put('/api/users/:uid/username', authenticateToken, requireSelf('uid'), async (req, res) => {
...
app.get('/api/users/:uid', async (req, res) => {
    try {
        const user = stripSensitive(await get('SELECT * FROM vuttik_users WHERE uid = ?', [req.params.uid]));
        if (user) {
            if (user.is_banned) {
...
app.get('/api/users/by-username/:username', async (req, res) => {
    try {
        const user = stripSensitive(await get('SELECT * FROM vuttik_users WHERE username = ? COLLATE NOCASE', [req.params.username]));
        if (user) {
            if (user.is_banned) {
                return res.status(403).json({ error: 'Tu cuenta ha sido suspendida.' });
            }
...
app.get('/api/users', authenticateToken, requireMegaGuardian, async (req, res) => {
    try {
...
app.put('/api/users/me/mode', authenticateToken, async (req, res) => {
    const { mode } = req.body;
...
function normalizarEnlacesSociales(entrada: any): { links: Record<string, string>, errores: string[] } {
...
app.put('/api/business-profiles/:uid', authenticateToken, requireBusinessAccess('uid'), async (req, res) => {
    ...
    try {
       ...
       working_hours=excluded.working_hours, social_links=excluded.social_links, logo=excluded.logo, updated_at=excluded.updated_at`, [
        ...
        try {
            const db = getDB();
            const existingPosBizIndex = db.businesses.findIndex((b) => b.id === uid);
...
app.get('/api/business-members/:businessId', authenticateToken, requireBusinessAccess('businessId'), async (req, res) => {
    try {
...
app.post('/api/business-members/invite', authenticateToken, async (req, res) => {
    const { businessUid, email } = req.body;
    if (!businessUid || !email)
        return res.status(400).json({ error: 'Missing params' });
    try {
        if (req.user.role !== 'mega_guardian' && !await userCanActForBusiness(req.user.uid, businessUid)) {
...
app.put('/api/business-members/:id/accept', authenticateToken, async (req, res) => {
    try {
        const member = await get('SELECT * FROM vuttik_business_members WHERE id = ?', [req.params.id]);
        if (!member)
            return res.status(404).json({ error: 'Invitación no encontrada' });
        if (member.member_uid !== req.user.uid)
...
app.put('/api/business-members/:id/role', authenticateToken, async (req, res) => {
    try {
        ...
        if (req.user.role !== 'mega_guardian') {
            const biz = await get('SELECT owner_uid FROM vuttik_business_profiles WHERE uid = ?', [member.business_uid]);
            if (!biz || biz.owner_uid !== req.user.uid)
        ...
        await logAction(req.user.uid, 'CHANGE_MEMBER_ROLE', member.member_uid, 'business_member', { newRole: role, businessUid: member.business_uid });
        res.json({ success: true });
    }
    catch (error) {
        res.status(500).json({ error: error.message });
...
app.delete('/api/business-members/:id', authenticateToken, async (req, res) => {
    try {
        const member = await get('SELECT * FROM vuttik_business_members WHERE id = ?', [req.params.id]);
        if (!member)
            return res.status(404).json({ error: 'Miembro no encontrado' });
        let allowed = member.member_uid === req.user.uid || req.user.role === 'mega_guardian';
        if (!allowed) {
            const biz = await get('SELECT owner_uid FROM vuttik_business_profiles WHERE uid = ?', [member.business_uid]);
            allowed = !!biz && biz.owner_uid === req.user.uid;
        }
        if (!allowed)
...
app.get('/api/users/:uid/business-invites', authenticateToken, requireSelf('uid'), async (req, res) => {
    try {
...
app.get('/api/users/:uid/businesses', authenticateToken, requireSelf('uid'), async (req, res) => {
    try {
       ...
       WHERE b.owner_uid = ? OR (m.member_uid = ? AND m.status = "accepted")`, [req.params.uid, req.params.uid]);
        res.json(rows);
    }
    catch (error) {
        res.status(500).json({ error: error.message });
...
app.post('/api/users', authenticateToken, async (req, res) => {
    const { displayName, photoURL, onboardingCompleted, age, gender, country, language, username } = req.body;
    // Identity, role and plan are server-owned: taking them from the body was a
...
app.post('/api/categories', authenticateToken, requireMegaGuardian, async (req, res) => {
...
app.delete('/api/categories/:id', authenticateToken, requireMegaGuardian, async (req, res) => {
    const { id } = req.params;
...
// Category Proposals
app.get('/api/categories/proposals', optionalAuth, async (req, res) => {
    // Used only to flag which proposals the viewer already voted on. Prefer the
    // token; the query parameter stays as a fallback for anonymous callers.
...
app.post('/api/categories/proposals', authenticateToken, async (req, res) => {
...
app.post('/api/categories/proposals/:id/vote', authenticateToken, async (req, res) => {
    const { id } = req.params;
...
app.post('/api/transaction-types', authenticateToken, requireMegaGuardian, async (req, res) => {
...
app.delete('/api/transaction-types/:id', authenticateToken, requireMegaGuardian, async (req, res) => {
    try {
...
app.post('/api/subscription-plans', authenticateToken, requireMegaGuardian, async (req, res) => {
...
app.delete('/api/subscription-plans/:id', authenticateToken, requireMegaGuardian, async (req, res) => {
    const { id } = req.params;
...
app.post('/api/notifications/:id/read', authenticateToken, async (req, res) => {
    try {
        // Scope by owner so one user cannot mark another's notification read.
        await run('UPDATE vuttik_notifications SET is_read = 1 WHERE id = ? AND user_id = ?', [req.params.id, req.user.uid]);
        res.json({ success: true });
    }
    catch (error) {
        res.status(500).json({ error: error.message });
...
app.post('/api/ean-database', authenticateToken, async (req, res) => {
    const data = req.body;
    try {
...
app.put('/api/ean-database/:ean', authenticateToken, async (req, res) => {
...
app.get('/api/products', async (req, res) => {
    ...
    const { categoryId, authorId, postedAs, q } = req.query;
    // Cache logic for global feed
    const isGlobalQuery = page === 1 && (!categoryId || categoryId === 'GLOBAL') && !authorId && !postedAs && !q;
    ...
    try {
        ...
        SELECT 
          ...
          p.chain, p.store_name, p.is_independent, p.country, p.province, p.stock, p.custom_fields,
      ...
      `;
        ...
        if (q) {
            conditions.push('(p.title LIKE ? OR p.author_name LIKE ? OR p.store_name LIKE ?)');
            params.push(`%${q}%`, `%${q}%`, `%${q}%`);
        }
...
app.get('/api/products/:id', async (req, res) => {
    ...
    `, [id]);
        ...
        let parsedDownVotes = [];
        try {
            parsedUpVotes = product.up_votes ? JSON.parse(product.up_votes).filter((v) => v !== null) : [];
        }
        catch (e) { }
        try {
            parsedDownVotes = product.down_votes ? JSON.parse(product.down_votes).filter((v) => v !== null) : [];
        }
        catch (e) { }
        res.json({
            ...product,
...
app.post('/api/products', authenticateToken, async (req, res) => {
    ...
    const data = req.body;
    try {
        // The client may ask to publish as a business, but only one it belongs to.
        data.authorId = await resolveActingUid(req, data.authorId);
        const user = await get('SELECT is_banned, strikes FROM vuttik_users WHERE uid = ?', [req.user.uid]);
        if (user?.is_banned)
            return res.status(403).json({ error: 'Tu cuenta ha sido suspendida.' });
        if (user?.strikes >= 3)
...
app.delete('/api/products/:id', authenticateToken, async (req, res) => {
    ...
    try {
        ...
        if (!await canEditContent(req, product.author_id)) {
...
app.put('/api/products/:id', authenticateToken, async (req, res) => {
    ...
    try {
        ...
        if (!existing)
            return res.status(404).json({ error: 'Product not found' });
        if (!await canEditContent(req, existing.author_id)) {
...
// Vote on product
app.post('/api/products/:id/vote', authenticateToken, async (req, res) => {
    const { id } = req.params;
...
app.post('/api/posts', authenticateToken, async (req, res) => {
    const { id, authorName, authorAvatar, content, image, postedAs } = req.body;
    try {
        const authorId = await resolveActingUid(req, req.body.authorId);
        // Verification is granted by moderators, never asserted by the client.
...
app.delete('/api/posts/:id', authenticateToken, async (req, res) => {
    ...
    try {
        ...
        if (!await canEditContent(req, post.author_id)) {
        ...
        // Log action
        if (userId)
            await logAction(userId, 'DELETE_POST', id, 'post', { snippet: post.content.substring(0, 50) });
    }
    catch (error) {
        res.status(500).json({ error: error.message });
    }
...
app.delete('/api/comments/:id', authenticateToken, async (req, res) => {
    ...
    const userId = req.user.uid;
    try {
        const comment = await get('SELECT author_id, post_id, content FROM vuttik_comments WHERE id = ?', [id]);
        if (!comment)
            return res.status(404).json({ error: 'Comment not found' });
        // The comment author may delete it, and so may the author of the post.
        let allowed = await canEditContent(req, comment.author_id);
        if (!allowed && comment.post_id) {
            const parent = await get('SELECT author_id FROM vuttik_posts WHERE id = ?', [comment.post_id]);
            allowed = !!parent && await canEditContent(req, parent.author_id);
        }
        if (!allowed)
...
// --- User Profile Update ---
app.put('/api/users/:uid/profile', authenticateToken, requireSelf('uid'), async (req, res) => {
    const { uid } = req.params;
...
app.put('/api/users/:uid/role', authenticateToken, requireMegaGuardian, async (req, res) => {
    const { uid } = req.params;
    const { role } = req.body;
    const adminId = req.user.uid;
    const ASSIGNABLE_ROLES = ['user', 'business', 'guardian', 'mega_guardian'];
    ...
    try {
        ...
        invalidateUser(uid);
        await logAction(adminId, 'CHANGE_USER_ROLE', uid, 'user', { newRole: role, oldRole: old?.role });
        res.json({ success: true });
    }
    catch (error) {
        res.status(500).json({ error: error.message });
...
// --- Unban User ---
app.post('/api/users/:uid/unban', authenticateToken, requireRole('mega_guardian', 'guardian'), async (req, res) => {
    const { uid } = req.params;
    const adminId = req.user.uid;
    try {
        await run('UPDATE vuttik_users SET is_banned = 0 WHERE uid = ?', [uid]);
        invalidateUser(uid);
        await logAction(adminId, 'UNBAN_USER', uid, 'user');
        res.json({ success: true });
    }
    catch (error) {
        res.status(500).json({ error: error.message });
...
app.post('/api/users/:uid/strike', authenticateToken, requireRole('mega_guardian', 'guardian'), async (req, res) => {
    const { uid } = req.params;
...
app.post('/api/posts/:postId/verify', authenticateToken, requireRole('mega_guardian', 'guardian'), async (req, res) => {
    const { postId } = req.params;
...
app.post('/api/reports', authenticateToken, async (req, res) => {
    const { targetId, targetType, targetTitle, authorId, authorName, reason } = req.body;
...
app.get('/api/reports', authenticateToken, requireMegaGuardian, async (req, res) => {
    try {
...
app.put('/api/reports/:id/status', authenticateToken, requireRole('mega_guardian', 'guardian'), async (req, res) => {
    const { id } = req.params;
...
// --- Admin Actions ---
app.post('/api/users/:uid/ban', authenticateToken, requireRole('mega_guardian', 'guardian'), async (req, res) => {
    const { uid } = req.params;
    const adminId = req.user.uid;
    try {
...
app.get('/api/stats/mega-guardian', authenticateToken, requireMegaGuardian, async (req, res) => {
    try {
...
// Trends (7 days)
app.get('/api/stats/trends', authenticateToken, requireMegaGuardian, async (req, res) => {
    try {
        const rows = await all(`
      SELECT date as name, searches as busquedas, views as vistas 
...
app.get('/api/stats/business/:userId', authenticateToken, requireBusinessAccess('userId'), async (req, res) => {
...
app.post('/api/follows', authenticateToken, async (req, res) => {
    ...
    try {
        ...
        res.json({ success: true });
        // Notification
        const followerUser = await get('SELECT display_name FROM vuttik_users WHERE uid = ?', [followerId]);
        const followerName = followerUser?.display_name || followerUser?.name || 'Un usuario';
...
// Unfollow a user
app.delete('/api/follows', authenticateToken, async (req, res) => {
    const { followingId } = req.body;
    const followerId = req.user.uid;
    if (!followingId)
        return res.status(400).json({ error: 'Missing params' });
...
app.post('/api/products/:id/follow', authenticateToken, async (req, res) => {
    const { id } = req.params;
    const { userId } = req.body;
    if (!userId)
...
app.delete('/api/products/:id/follow', authenticateToken, async (req, res) => {
    const { id } = req.params;
...
app.get('/api/users/:uid/following-products', authenticateToken, requireSelf('uid'), async (req, res) => {
    try {
        const rows = await all(`
      ...
      `;
            ...
            if (filter === 'following' && userId) {
                ...
                if (following.length > 0) {
                    ...
                    queryParams = following.map((f) => f.following_id);
                }
                else {
                    query += ` WHERE 1=0`;
                }
...
app.get('/api/users/:uid/unread-messages', authenticateToken, requireSelf('uid'), async (req, res) => {
    try {
        const uid = req.params.uid;
...
app.get('/api/conversations/:userId', authenticateToken, requireSelf('userId'), async (req, res) => {
    try {
...
app.post('/api/conversations', authenticateToken, async (req, res) => {
    const { userId2, p1Name, p1Photo, p2Name, p2Photo } = req.body;
    const userId1 = req.user.uid;
    if (!userId2)
        return res.status(400).json({ error: 'Missing params' });
    if (userId1 === userId2)
...
app.get('/api/messages/:conversationId', authenticateToken, async (req, res) => {
    try {
        if (!await isConversationParticipant(req.params.conversationId, req.user.uid)) {
            return res.status(403).json({ error: 'Acceso denegado' });
        }
...
app.post('/api/messages', authenticateToken, async (req, res) => {
    ...
    if (!conversationId || !content)
        return res.status(400).json({ error: 'Missing params' });
    try {
        if (!await isConversationParticipant(conversationId, senderId)) {
            return res.status(403).json({ error: 'Acceso denegado' });
        }
        const id = uuidv4();
        const now = new Date().toISOString();
...
app.get('/api/audit-logs', authenticateToken, requireMegaGuardian, async (req, res) => {
    const limit = parseInt(String(req.query.limit)) || 300;
...
app.get('/api/portfolios', authenticateToken, async (req, res) => {
    // Only ever list the caller's own portfolios.
    const userId = req.user.uid;
    try {
...
app.put('/api/portfolios/:id/products', authenticateToken, async (req, res) => {
    const { products } = req.body;
    const portfolioId = req.params.id;
    try {
        const now = new Date().toISOString();
        // Scoping the UPDATE by user_id makes another user's id a no-op.
        await run(`UPDATE vuttik_portfolios SET products = ?, updated_at = ? WHERE id = ? AND user_id = ?`, [JSON.stringify(products || []), now, portfolioId, req.user.uid]);
        res.json({ success: true });
    }
    catch (e) {
        res.status(500).json({ error: e.message });

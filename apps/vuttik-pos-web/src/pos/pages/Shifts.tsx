...
export default function Shifts() {
  ...
  // Auto-open specific shift if navigated from a notification
  useEffect(() => {
    if (location.state?.viewShiftId && shifts.length > 0) {
      const shiftToView = shifts.find(s => s.id === location.state.viewShiftId);
      if (shiftToView) {
        setSelectedShift(shiftToView);
        // Clear the state so it doesn't re-trigger on refresh
        window.history.replaceState({}, document.title);
      }
    }
  }, [location.state, shifts]);

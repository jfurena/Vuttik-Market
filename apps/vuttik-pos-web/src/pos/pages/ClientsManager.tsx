...
export default function ClientsManager() {
  ...
  const [loyaltyConfig, setLoyaltyConfig] = useState<any>({ activa: false, tipo_regla: 'monto', meta: 1000, recompensa_credito: 100, mes_evaluacion: 'historico' });
  ...
  const handleAddClient = async (e: React.FormEvent) => {
    ...
    try {
      const newClient = await ApiService.createCliente({
        ...
        limite_credito: clientForm.permitir_fiao ? (Number(clientForm.limite_credito) || 0) : 0
      });
      
      setShowAddModal(false);
      setClientForm({ nombre: '', telefono: '', limite_credito: '', permitir_fiao: false });
      loadData();
  ...
  const handleEditClient = async (e: React.FormEvent) => {
    ...
    try {
      const updated = await ApiService.updateCliente(editClientForm.id, {
        ...
        limite_credito: editClientForm.permitir_fiao ? (Number(editClientForm.limite_credito) || 0) : 0,
  ...
  const handleSaveLoyalty = async () => {
    ...
    try {
      await ApiService.updateLoyaltyConfig(loyaltyConfig);
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
        ...
        <div className="flex gap-2">
          <button 
            ...
            className="bg-purple-100 text-purple-700 hover:bg-purple-200 px-6 py-3.5 rounded-2xl font-black flex items-center gap-2 transition-all duration-300 shadow-sm"
          >
          ...
          <button 
            ...
            className="bg-gradient-to-r from-blue-600 to-indigo-600 text-white px-6 py-3.5 rounded-2xl font-black flex items-center gap-2 hover:from-blue-500 hover:to-indigo-500 transition-all duration-300 shadow-lg shadow-blue-950/40 hover:-translate-y-0.5"
          >
              ...
              <div className="space-y-4">
                ...
                <div className="border-b border-gray-100 pb-4 space-y-2">
                  ...
                  <div className="grid grid-cols-2 gap-3 mt-4">
                    ...
                    <div className="bg-white/60 p-3 rounded-2xl border border-gray-100">
                      <span className="text-[10px] font-semibold text-gray-500 uppercase block tracking-wider">Límite (Fiao + Lealtad)</span>
                      <span className="text-base font-black text-gray-900">
                        {((selectedCliente as any).limite_credito_dinamico ?? selectedCliente.limite_credito) > 0 ? formatCurrency((selectedCliente as any).limite_credito_dinamico ?? selectedCliente.limite_credito) : 'Sin Límite'}
                      </span>
                    </div>
                  </div>
                </div>
                ...
                {(() => {
                  const historicalSales = history.filter(h => h.tipo === 'venta' && h.estado !== 'cancelada' && h.estado !== 'reembolsada');
                  const totalVisits = historicalSales.length;
                  const totalSpent = historicalSales.reduce((acc, s) => acc + (s.total || 0), 0);

                  let periodSales = historicalSales;
                  if (loyaltyConfig.activa && loyaltyConfig.mes_evaluacion !== 'historico') {
                    if (loyaltyConfig.mes_evaluacion.includes('|')) {
                      const [start, end] = loyaltyConfig.mes_evaluacion.split('|');
                      periodSales = periodSales.filter(s => {
                        const d = s.fecha ? s.fecha.slice(0, 10) : '';
                        return (!start || d >= start) && (!end || d <= end);
                      });
                    } else {
                      periodSales = periodSales.filter(s => s.fecha && s.fecha.startsWith(loyaltyConfig.mes_evaluacion));
                    }
                  }
                  
                  const val = loyaltyConfig.tipo_regla === 'monto' 
                    ? periodSales.reduce((acc, s) => acc + (s.total || 0), 0)
                    : periodSales.length;
                  
                  const target = loyaltyConfig.meta || 1;
                  const pct = Math.min(100, (val / target) * 100);
                  const achieved = val >= target;
                  const creditEarned = achieved ? loyaltyConfig.recompensa_credito : 0;
                    ...
                    <div className="space-y-3 pb-4 border-b border-gray-100">
                      ...
                      <div className="grid grid-cols-2 gap-3">
                        <div className="bg-blue-50/50 p-3 rounded-2xl border border-blue-100">
                          <span className="text-[10px] font-semibold text-blue-600 uppercase block tracking-wider">Total Gastado</span>
                          <span className="text-base font-black text-blue-900">{formatCurrency(totalSpent)}</span>
                        </div>
                        <div className="bg-blue-50/50 p-3 rounded-2xl border border-blue-100">
                          <span className="text-[10px] font-semibold text-blue-600 uppercase block tracking-wider">Visitas (Compras)</span>
                          <span className="text-base font-black text-blue-900">{totalVisits}</span>
                        </div>
                      </div>

                      {/* Progreso de Promoción (Solo si está activa) */}
                      {loyaltyConfig.activa && (
                        <div className="bg-gradient-to-br from-purple-50 to-indigo-50/50 p-4 rounded-2xl border border-purple-100 space-y-3">
                          <div className="flex justify-between items-center">
                            ...
                            <span className="text-[10px] font-bold bg-white px-2 py-0.5 rounded text-purple-600 shadow-sm border border-purple-100">
                              {loyaltyConfig.mes_evaluacion === 'historico' ? 'Histórico' : (loyaltyConfig.mes_evaluacion.includes('|') ? 'Campaña Temporal' : `Mes: ${loyaltyConfig.mes_evaluacion}`)}
                            </span>
                          </div>
                          
                          <div>
                            <div className="flex justify-between text-[10px] font-bold text-purple-800 mb-1.5">
                              <span>
                                {loyaltyConfig.tipo_regla === 'monto' 
                                  ? `Acumulado: ${formatCurrency(val)}`
                                  : `Visitas: ${val}`}
                              </span>
                              <span>Meta: {loyaltyConfig.tipo_regla === 'monto' ? formatCurrency(target) : target}</span>
                            </div>
                            <div className="w-full bg-white/60 rounded-full h-2 overflow-hidden shadow-inner border border-purple-100/50">
                              <div 
                                className={`h-full rounded-full transition-all duration-1000 ${achieved ? 'bg-emerald-500' : 'bg-purple-500'}`}
                                style={{ width: `${pct}%` }}
                              />
                            </div>
                          </div>
                          ...
                          {achieved && (
                            <div className="bg-emerald-50 text-emerald-800 p-2 rounded-lg text-[10px] font-bold flex items-center gap-1.5 border border-emerald-100">
                              <CheckCircle className="h-3 w-3 text-emerald-500" />
                              ¡Alcanzó la meta! Gana +{formatCurrency(creditEarned)} en crédito.
                            </div>
                          )}
                        </div>
                      )}
                          ...
                          >
                            <div className="space-y-1">
                              <div className="flex items-center gap-1.5">
                                <span className={`p-1 rounded-md ${isPayment ? 'bg-emerald-500/10 text-emerald-400' : (item.metodo_pago && item.metodo_pago !== 'A Crédito (Fiao)' ? 'bg-blue-500/10 text-blue-400' : 'bg-red-500/10 text-red-400')}`}>
                                  {isPayment ? <ArrowDownLeft className="h-3 w-3" /> : <ArrowUpRight className="h-3 w-3" />}
                                </span>
                                <span className="font-bold text-gray-900 group-hover:text-blue-600 transition-colors">
                                  {isPayment ? 'Abono / Pago' : (item.metodo_pago && item.metodo_pago !== 'A Crédito (Fiao)' ? `Venta: ${item.metodo_pago}` : 'Venta Fiada')}
                                </span>
                              </div>
        ...
        <div className="fixed inset-0 bg-black/60 backdrop-blur-sm flex items-center justify-center p-4 z-50">
          <div className="bg-white p-8 rounded-3xl max-w-lg w-full shadow-2xl">
            <div className="flex justify-between items-center mb-6">
              <div>
                <h3 className="text-2xl font-black text-gray-900 tracking-tight">Campaña de Fidelidad</h3>
                <p className="text-xs font-semibold text-gray-500 uppercase">Reglas para ganar límite de fiao</p>
              </div>
              <button onClick={() => setShowLoyaltyModal(false)} className="text-gray-400 hover:text-gray-900 bg-gray-100 hover:bg-gray-200 p-2 rounded-xl transition-colors">
                <X className="h-5 w-5" />
              </button>
            </div>
            
            <div className="space-y-4">
              <label className="flex items-center gap-3 bg-purple-50 p-4 rounded-xl border border-purple-100 cursor-pointer">
                <input 
                  type="checkbox" 
                  checked={loyaltyConfig.activa} 
                  onChange={e => setLoyaltyConfig({...loyaltyConfig, activa: e.target.checked})}
                  className="w-5 h-5 accent-purple-600"
                />
                <span className="font-bold text-purple-900">Activar Programa de Lealtad</span>
              </label>

              {loyaltyConfig.activa && (
                <div className="space-y-4 bg-gray-50 p-4 rounded-xl border border-gray-100">
                  <div>
                    <label className="block text-xs font-bold text-gray-700 uppercase mb-1">Método de Evaluación</label>
                    <select 
                      value={loyaltyConfig.tipo_regla}
                      onChange={e => setLoyaltyConfig({...loyaltyConfig, tipo_regla: e.target.value})}
                      className="w-full px-4 py-3 bg-white border border-gray-200 rounded-xl focus:ring-2 focus:ring-blue-500 font-medium"
                    >
                      <option value="monto">Por Monto Gastado Acumulado</option>
                      <option value="visitas">Por Cantidad de Compras (Visitas)</option>
                    </select>
                  </div>
                  ...
                  <div className="grid grid-cols-2 gap-4">
                    <div>
                      <label className="block text-xs font-bold text-gray-700 uppercase mb-1">
                        Meta ({loyaltyConfig.tipo_regla === 'monto' ? 'Monto $' : 'Compras'})
                      </label>
                      <input 
                        type="number" 
                        value={loyaltyConfig.meta}
                        onChange={e => setLoyaltyConfig({...loyaltyConfig, meta: Number(e.target.value)})}
                        className="w-full px-4 py-3 bg-white border border-gray-200 rounded-xl focus:ring-2 focus:ring-blue-500 font-bold"
                        min="1"
                      />
                    </div>
                    <div>
                      <label className="block text-xs font-bold text-gray-700 uppercase mb-1">Premio Límite ($)</label>
                      <input 
                        type="number" 
                        value={loyaltyConfig.recompensa_credito}
                        onChange={e => setLoyaltyConfig({...loyaltyConfig, recompensa_credito: Number(e.target.value)})}
                        className="w-full px-4 py-3 bg-white border border-gray-200 rounded-xl focus:ring-2 focus:ring-blue-500 font-bold"
                        min="0"
                      />
                    </div>
                  ...
                  <div>
                    <label className="block text-xs font-bold text-gray-700 uppercase mb-2">Duración de la Campaña</label>
                    <div className="flex bg-gray-100 p-1 rounded-xl mb-3">
                      <button
                        type="button"
                        onClick={(e) => {
                          e.preventDefault();
                          setLoyaltyConfig({...loyaltyConfig, mes_evaluacion: 'historico'});
                          setShowMonthPicker(false);
                        }}
                        className={`flex-1 py-2 text-xs font-bold rounded-lg transition-all ${loyaltyConfig.mes_evaluacion === 'historico' ? 'bg-white text-gray-900 shadow-sm' : 'text-gray-500 hover:text-gray-700'}`}
                      >
                      ...
                      <button
                        ...
                        onClick={(e) => {
                          e.preventDefault();
                          const now = new Date();
                          setLoyaltyConfig({...loyaltyConfig, mes_evaluacion: `${now.getFullYear()}-${String(now.getMonth()+1).padStart(2, '0')}`});
                        }}
                        className={`flex-1 py-2 text-xs font-bold rounded-lg transition-all ${loyaltyConfig.mes_evaluacion !== 'historico' ? 'bg-white text-gray-900 shadow-sm' : 'text-gray-500 hover:text-gray-700'}`}
                      >
                        Mensual (Temporal)
                    ...
                    {loyaltyConfig.mes_evaluacion !== 'historico' ? (
                      <div className="relative animate-in fade-in slide-in-from-top-1 space-y-2">
                        <label className="block text-[10px] font-bold text-gray-400 uppercase mb-1">Periodo de la Campaña</label>
                        <div className="flex gap-2">
                          <div className="flex-1">
                            <span className="text-[10px] text-gray-500 font-bold block mb-1">Fecha Inicio</span>
                            <input 
                              type="date"
                              value={loyaltyConfig.mes_evaluacion.includes('|') ? loyaltyConfig.mes_evaluacion.split('|')[0] : (loyaltyConfig.mes_evaluacion + '-01')}
                              onChange={(e) => {
                                const end = loyaltyConfig.mes_evaluacion.includes('|') ? loyaltyConfig.mes_evaluacion.split('|')[1] : '';
                                setLoyaltyConfig({...loyaltyConfig, mes_evaluacion: `${e.target.value}|${end}`});
                              }}
                              className="w-full px-3 py-2.5 bg-white border border-gray-200 rounded-xl font-medium focus:outline-none focus:border-blue-500 text-sm transition-colors"
                            />
                          </div>
                          <div className="flex-1">
                            <span className="text-[10px] text-gray-500 font-bold block mb-1">Fecha Fin</span>
                            <input 
                              type="date"
                              value={loyaltyConfig.mes_evaluacion.includes('|') ? loyaltyConfig.mes_evaluacion.split('|')[1] : ''}
                              onChange={(e) => {
                                const start = loyaltyConfig.mes_evaluacion.includes('|') ? loyaltyConfig.mes_evaluacion.split('|')[0] : '';
                                setLoyaltyConfig({...loyaltyConfig, mes_evaluacion: `${start}|${e.target.value}`});
                              }}
                              className="w-full px-3 py-2.5 bg-white border border-gray-200 rounded-xl font-medium focus:outline-none focus:border-blue-500 text-sm transition-colors"
                            />
                          </div>
                      ...
                      <p className="text-[10px] text-gray-500 bg-gray-50 p-2 rounded-lg border border-gray-100 italic">
                        La campaña evaluará el historial completo del cliente sin reiniciarse cada mes.
                      </p>
                    )}
                  </div>
                </div>
              ...
              <button 
                ...
                className="w-full py-4 bg-purple-600 hover:bg-purple-700 text-white rounded-xl font-black tracking-wide uppercase transition-colors shadow-lg shadow-purple-600/30"
              >
                {savingLoyalty ? 'Guardando...' : 'Guardar Campaña'}
              </button>
            </div>
          </div>
        </div>
        ...
        <div className="fixed inset-0 bg-black/50 flex items-center justify-center p-6 z-50 overflow-y-auto">
          <div className="bg-white border border-gray-100 rounded-[2rem] w-full max-w-2xl shadow-2xl overflow-hidden my-8">
            ...
            <form onSubmit={handleAddClient} className="p-8 space-y-6">
              ...
              {/* Límite de Crédito */}
              <div className="space-y-4">
                <label className="flex items-center gap-3 bg-gray-50 p-4 rounded-xl border border-gray-100 cursor-pointer">
                  <input 
                    type="checkbox" 
                    checked={clientForm.permitir_fiao} 
                    onChange={e => setClientForm({...clientForm, permitir_fiao: e.target.checked})}
                    className="w-5 h-5 accent-blue-600 rounded"
                  />
                  <div>
                    <span className="font-bold text-gray-900 block text-sm">¿Apto para fiar manualmente?</span>
                    <span className="text-[10px] text-gray-500">Otorgarle un límite de crédito fijo sin depender de la lealtad.</span>
                  </div>
                </label>

                {clientForm.permitir_fiao && (
                  <div className="space-y-2 p-4 border border-blue-100 bg-blue-50/30 rounded-xl">
                    ...
                    <div className="relative">
                      ...
                      <input 
                        type="number"
                        min="0"
                        placeholder="Ej: 10000 (Coloca 0 para ilimitado)"
                        value={clientForm.limite_credito}
                        onChange={(e) => setClientForm({ ...clientForm, limite_credito: e.target.value })}
                        className="w-full pl-12 pr-4 py-3 bg-white border border-blue-100 rounded-xl text-gray-900 placeholder-gray-400 focus:outline-none focus:border-blue-500/50 text-sm transition-all shadow-sm"
                      />
                    </div>
                    <p className="text-[10px] text-gray-500 mt-1 leading-relaxed">
                      El POS bloqueará ventas si su deuda total excede este límite configurado más su crédito por fidelidad.
                    </p>
                  </div>
                )}
              </div>
            ...
            <form onSubmit={handleEditClient} className="p-8 space-y-6">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div className="space-y-4 md:col-span-2">
                  <label className="flex items-center gap-3 bg-gray-50 p-4 rounded-xl border border-gray-100 cursor-pointer w-full">
                    <input 
                      type="checkbox" 
                      checked={editClientForm.permitir_fiao} 
                      onChange={e => setEditClientForm({...editClientForm, permitir_fiao: e.target.checked})}
                      className="w-5 h-5 accent-blue-600 rounded"
                    />
                    <div>
                      <span className="font-bold text-gray-900 block text-sm">¿Apto para fiar manualmente?</span>
                    ...
                    <div className="space-y-2 p-4 border border-blue-100 bg-blue-50/30 rounded-xl">
                      ...
                      <div className="relative">
                        ...
                        <input 
                          ...
                          className="w-full pl-12 pr-4 py-3 bg-white border border-blue-100 rounded-xl text-gray-900 focus:outline-none focus:border-blue-500/50 text-sm transition-all shadow-sm"
                        />
                      </div>
                    </div>
                  )}

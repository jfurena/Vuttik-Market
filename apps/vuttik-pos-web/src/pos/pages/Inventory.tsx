...
import { Plus, Search, Filter, AlertTriangle, Edit2, Package, RefreshCw, Wand2, Loader2, ShoppingCart, History, TrendingUp, ArrowRightLeft , Image as ImageIcon } from 'lucide-react';
import { useAuth } from '../context/AuthContext';
import { formatCurrency, cn, getActiveProductPrice } from '../lib/utils';
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        ...
        <div className="flex gap-2 flex-col sm:flex-row">
          <button 
            ...
            className="bg-blue-600 text-white px-6 py-4 rounded-2xl font-black flex items-center justify-center gap-2 hover:bg-blue-700 transition-all shadow-xl shadow-blue-100 uppercase tracking-widest text-xs"
          >
                    ...
                    <td className="px-6 py-4">
                      <div className="flex items-center gap-3">
                        {(product as any).imagen ? (
                          <img src={(product as any).imagen} className="h-10 w-10 rounded-lg object-cover shadow-sm" />
                        ) : (
                        ...
                        return (
                          <div className="flex flex-col gap-0.5">
                            <p className="font-bold text-blue-600 flex items-center gap-2">
                              ...
                              {isDiscounted && <span className="bg-orange-100 text-orange-600 text-[9px] px-1.5 py-0.5 rounded-full font-black uppercase shadow-sm border border-orange-200">Oferta</span>}
                            </p>
                            {isDiscounted && <p className="text-[10px] text-gray-400 line-through">{formatCurrency(product.precio_venta)}</p>}
                          </div>
                        );
                      })()}
        ...
        <div className="fixed inset-0 bg-black/50 flex items-center justify-center p-4 z-50">
          ...
          >
            ...
            <form onSubmit={handleSubmit} className="grid grid-cols-1 md:grid-cols-2 gap-4">
              ...
              <div className="md:col-span-2">
  <label className="block text-[10px] font-black text-gray-400 uppercase tracking-widest mb-2">Imagen del Producto</label>
  <div className="flex items-center gap-4 mb-4">
    {formData.imagen ? (
      <img src={formData.imagen} className="h-16 w-16 rounded-xl object-cover shadow-sm border border-gray-200" />
    ) : (
      <div className="h-16 w-16 bg-gray-100 border-2 border-dashed border-gray-200 rounded-xl flex items-center justify-center text-gray-400">
...
<label className="block text-[10px] font-black text-gray-400 uppercase tracking-widest mb-2">Nombre del Producto</label>
              ...
              <div>
                ...
                <input required type="number" step="0.01" placeholder="0.00" className="w-full px-6 py-4 bg-gray-50 border-2 border-gray-100 rounded-2xl focus:border-blue-500 outline-none font-bold animate-pulse focus:animate-none border-blue-100" value={formData.precio_venta} onChange={e => setFormData({...formData, precio_venta: Number(e.target.value)})} />
                              ...
                              <div className="md:col-span-2 p-4 bg-emerald-50 border border-emerald-100 rounded-3xl flex flex-col gap-4">
                <div className="flex items-center justify-between">
                  <div>
                    <h4 className="text-sm font-black text-emerald-900">Incentivo / Comisión por Venta</h4>
                    <p className="text-[10px] font-bold text-emerald-700 mt-1">Si activas esto, el empleado ganará un porcentaje por cada unidad que venda.</p>
                  </div>
                  <button
                    type="button"
                    onClick={() => setFormData({...formData, comision_activa: !formData.comision_activa})}
                    className={`relative inline-flex h-8 w-14 items-center rounded-full transition-colors ${formData.comision_activa ? 'bg-emerald-600' : 'bg-emerald-200'}`}
                  >
                    <span className={`inline-block h-6 w-6 transform rounded-full bg-white transition-transform ${formData.comision_activa ? 'translate-x-7' : 'translate-x-1'}`} />
                  </button>
                </div>
                {formData.comision_activa && (
                  <div>
                    <label className="block text-[10px] font-black text-emerald-600 uppercase tracking-widest mb-2">Porcentaje de Comisión (%)</label>
                    <input type="number" step="0.01" min="0.01" max="100" placeholder="Ej. 10 para 10%" className="w-full px-4 py-3 bg-white border border-emerald-200 rounded-xl focus:border-emerald-500 outline-none font-bold text-emerald-900" value={formData.comision_porcentaje || ''} onChange={e => setFormData({...formData, comision_porcentaje: Number(e.target.value)})} />
                  </div>
                )}
              </div>
              ...
              {/* Oferta Temporal */}
              <div className="md:col-span-2 p-4 bg-orange-50 border border-orange-100 rounded-3xl flex flex-col gap-4">
                <div className="flex items-center justify-between">
                  <div>
                    <h4 className="text-sm font-black text-orange-900">Oferta Temporal / Descuento</h4>
                    <p className="text-[10px] font-bold text-orange-700 mt-1">Configura un precio especial durante un periodo de tiempo.</p>
                  </div>
                  <button
                    type="button"
                    onClick={() => setFormData({...formData, oferta_activa: !formData.oferta_activa})}
                    className={`relative inline-flex h-8 w-14 items-center rounded-full transition-colors ${formData.oferta_activa ? 'bg-orange-600' : 'bg-orange-200'}`}
                  >
                    <span className={`inline-block h-6 w-6 transform rounded-full bg-white transition-transform ${formData.oferta_activa ? 'translate-x-7' : 'translate-x-1'}`} />
                  </button>
                </div>
                {formData.oferta_activa && (
                  <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                    <div>
                      <label className="block text-[10px] font-black text-orange-600 uppercase tracking-widest mb-2">Precio de Oferta (RD$)</label>
                      <input type="number" step="0.01" min="0" className="w-full px-4 py-3 bg-white border border-orange-200 rounded-xl focus:border-orange-500 outline-none font-bold text-orange-900" value={formData.precio_oferta || ''} onChange={e => setFormData({...formData, precio_oferta: Number(e.target.value)})} />
                    </div>
                    <div>
                      <label className="block text-[10px] font-black text-orange-600 uppercase tracking-widest mb-2">Fecha Inicio</label>
                      <input type="date" className="w-full px-4 py-3 bg-white border border-orange-200 rounded-xl focus:border-orange-500 outline-none font-medium text-orange-900" value={formData.oferta_inicio || ''} onChange={e => setFormData({...formData, oferta_inicio: e.target.value})} />
                    </div>
                    <div>
                      <label className="block text-[10px] font-black text-orange-600 uppercase tracking-widest mb-2">Fecha Fin</label>
                      <input type="date" className="w-full px-4 py-3 bg-white border border-orange-200 rounded-xl focus:border-orange-500 outline-none font-medium text-orange-900" value={formData.oferta_fin || ''} onChange={e => setFormData({...formData, oferta_fin: e.target.value})} />
                    </div>
                  </div>
                )}
              </div>

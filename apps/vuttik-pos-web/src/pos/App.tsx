...
function RequireAdmin({ children, permission }: { children: React.ReactNode, permission?: string }) {
  const { profile, loading } = useAuth();
  if (loading) return null;
  if (!profile?.business_id) {
     console.error('RequireAdmin REDIRECTING! profile:', profile);
     return <Navigate to="/businesses" />;
  }
  if (profile.rol !== UserRole.ADMIN) {
     if (permission && profile.permisos && profile.permisos.includes(permission)) {
       return <>{children}</>;
     }

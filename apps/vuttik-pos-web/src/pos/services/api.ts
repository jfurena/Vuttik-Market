...
const isPosDomain = window.location.hostname.includes('pos') || window.location.pathname.startsWith('/pos');
const isDevelopment = window.location.hostname === 'localhost' || window.location.hostname === '127.0.0.1';
const isPosSubdomain = window.location.hostname.includes('pos');
...
async function apiFetch(url: string, options?: RequestInit): Promise<Response> {
    ...
    const businessId = localStorage.getItem('vuttik_business_id');
    if (businessId) {
      fetchOptions.headers = {
        ...fetchOptions.headers,
  ...
  if (res.status === 401) {
    ...
    try {
      ...
      if (typeof data?.error === 'string' && data.error.includes('sesión de negocio')) {
        console.error('REDIRECTING TO /businesses! URL:', url, 'Data:', data);
        alert('API FETCH REDIRECT BLOCKED! URL: ' + url);
      }
    } catch (_) {}
  }
  return res;
...
export const ApiService = {
  ...
  async login(correo: string, password: string, location?: { lat: number, lng: number, address?: string }) {
    const res = await apiFetch(`${API_BASE}/auth/login`, {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ email: correo, password, location })
    });
  ...
  async employeeLogin(business_codigo: string, username: string, password: string) {
    const res = await apiFetch(`${API_BASE}/auth/employee-login`, {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
  ...
  async googleCallback(data: any) {
    const res = await apiFetch(`${API_BASE}/auth/google/callback`, {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
  ...
  async facebookCallback(data: any) {
    const res = await apiFetch(`${API_BASE}/auth/facebook/callback`, {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify(data)
    });
  ...
  async loginMe() {
    const token = localStorage.getItem('vuttik_token');
    const res = await apiFetch(`${API_BASE}/pos/me?t=${Date.now()}`, {
      headers: {
  ...
  async createBusiness(nombre: string, location?: any, logo?: string, description?: string, working_hours?: string, phone?: string, instagram?: string, facebook?: string, twitter?: string, website?: string, market_sync_url?: string, market_api_key?: string) {
    const social_links = { instagram, facebook, twitter, website };
    const res = await apiFetch(`${API_BASE}/businesses`, {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ nombre, location, logo, description, working_hours, phone, social_links, market_sync_url, market_api_key })
    });
    if (!res.ok) { 
  ...
  async requestMultiBusiness(data?: { nombre?: string, location?: any, logo?: string }) {
    const res = await apiFetch(`${API_BASE}/pos/request-multi-business`, {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify(data || {})
    });
  ...
  async dismissRejectedBusinessRequest(request_id: string) {
    const res = await apiFetch(`${API_BASE}/businesses/dismiss-rejected`, {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ request_id })
    });
    if (!res.ok) { const err = await res.json().catch(() => ({})); throw new Error(err.error || 'Error al limpiar la solicitud.'); }
    return res.json();
  },

  async claimApprovedBusinessRequest(request_id: string) {
    const res = await apiFetch(`${API_BASE}/businesses/claim-approved`, {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ request_id })
    });
    if (!res.ok) { const err = await res.json().catch(() => ({})); throw new Error(err.error || 'Error al procesar la solicitud aprobada.'); }
    return res.json();
  },

  async updateBusiness(id: string, nombre: string, location?: any, logo?: string, description?: string, working_hours?: string, phone?: string, instagram?: string, facebook?: string, twitter?: string, website?: string, market_sync_url?: string, market_api_key?: string) {
    if (isPracticeModeActive()) throw new Error('No se pueden editar negocios en modo práctica.');
    const social_links = { instagram, facebook, twitter, website };
    const res = await apiFetch(`${API_BASE}/businesses/${id}`, {
      method: 'PUT',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ nombre, location, logo, description, working_hours, phone, social_links, market_sync_url, market_api_key })
    });
  ...
  async deleteBusiness(id: string) {
    const res = await apiFetch(`${API_BASE}/businesses/${id}`, { method: 'DELETE' });
  ...
  async selectBusiness(business_id: string) {
    const res = await apiFetch(`${API_BASE}/auth/select-business`, {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ business_id })
    });
    if (!res.ok) { const err = await res.json().catch(() => ({})); throw new Error(err.error || 'Error al seleccionar negocio.'); }
    localStorage.setItem('vuttik_business_id', business_id);
    return res.json();
  },

  async exitBusiness() {
    const res = await apiFetch(`${API_BASE}/auth/exit-business`, { method: 'POST' });
    localStorage.removeItem('vuttik_business_id');
    return res.json();
  },
  ...
  async deleteEmployee(empId: string) {
    const res = await apiFetch(`${API_BASE}/employees/${empId}`, { method: 'DELETE' });
    return res.json();
  },
  ...
  async adjustProductStock(id: string, data: { cantidad: number, tipo_movimiento: string, motivo: string, usuario_id: string, metadata?: any }): Promise<Product> {
    ...
    const res = await apiFetch(`${API_BASE}/products/${id}/stock`, {
      method: 'PATCH',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify(data)
    });
  ...
  async restockProduct(id: string, data: { cantidad: number, costo_unitario: number, motivo: string, usuario_id: string, usuario_nombre: string, fuente_pago: 'Caja' | 'Banco' | 'Inversion Externa', metadata?: any }): Promise<Product> {
    ...
    const res = await apiFetch(`${API_BASE}/products/${id}/restock`, {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify(data)
    });
  ...
  async getProductHistory(id: string): Promise<any[]> {
    ...
    const res = await apiFetch(`${API_BASE}/products/${id}/history`);
    return res.json();
  },
  ...
  async getActiveShift(userId: string): Promise<Shift | null> {
    ...
    const res = await apiFetch(`${API_BASE}/shifts/active/${userId}`);
    return res.json();
  },
  ...
  async openShift(userId: string, userName: string, montoInicial: number): Promise<Shift> {
    ...
    const res = await apiFetch(`${API_BASE}/shifts/open`, {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
  ...
  async closeShift(shiftId: string, montoContado: number, desglose: any, motivoDiferencia: string): Promise<Shift> {
    ...
    const res = await apiFetch(`${API_BASE}/shifts/close/${shiftId}`, {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
  ...
  async getAllShifts(params?: { userId?: string, status?: string, date?: string }): Promise<Shift[]> {
    ...
    const res = await apiFetch(`${API_BASE}/shifts?${query}`);
    return res.json();
  },
  ...
  async updateShiftStatus(shiftId: string, data: { status: string, notaAdmin?: string, reviewedBy?: string }): Promise<Shift> {
    ...
    const res = await apiFetch(`${API_BASE}/shifts/${shiftId}/status`, {
      method: 'PATCH',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify(data)
    });
  ...
  async getCashMovements(shiftId: string): Promise<CashMovement[]> {
    ...
    const res = await apiFetch(`${API_BASE}/cash-movements/${shiftId}`);
    return res.json();
  },
  ...
  async addCashMovement(movement: Omit<CashMovement, 'id' | 'fecha'>): Promise<CashMovement> {
    ...
    const res = await apiFetch(`${API_BASE}/cash-movements`, {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
  ...
  async getStats(month?: number, year?: number, startDate?: string | null, endDate?: string | null) {
    ...
    const res = await apiFetch(`${API_BASE}/stats?${params.toString()}`);
    return res.json();
  },
  ...
  async getExpenses(): Promise<Expense[]> {
    ...
    const res = await apiFetch(`${API_BASE}/expenses`, { cache: 'no-store' });
    return res.json();
  },
  ...
  async refundSale(saleId: string, password?: string, motivo?: string, usuarioNombre?: string, usuarioId?: string): Promise<any> {
    ...
    const res = await apiFetch(`${API_BASE}/sales/refund/${saleId}`, {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
  ...
  async cancelSale(saleId: string): Promise<Sale> {
    ...
    const res = await apiFetch(`${API_BASE}/sales/cancel/${saleId}`, {
      method: 'POST'
    });
    return res.json();
  },
  ...
  async getInventoryMovements(): Promise<any[]> {
    ...
    const res = await apiFetch(`${API_BASE}/inventory/movements`);
    return res.json();
  },
  ...
  async getActivityLog(): Promise<any[]> {
    ...
    const res = await apiFetch(`${API_BASE}/activity-log`);
    return res.json();
  },
  ...
  async postActivityLog(data: any): Promise<void> {
    ...
    await apiFetch(`${API_BASE}/activity-log`, {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify(data)
    });
  ...
  async getApprovalRequests() {
    ...
    const res = await apiFetch(`${API_BASE}/approval-requests`);
    return res.json();
  },
  ...
  async getApprovalStatus(userId: string) {
    ...
    const res = await apiFetch(`${API_BASE}/approval-requests/status/${userId}`);
    return res.json();
  },
  ...
  async actionApprovalRequest(id: string, action: 'aprobar' | 'rechazar', options: { guardarEnListaBlanca: boolean, adminId: string, adminNombre: string }) {
    ...
    const res = await apiFetch(`${API_BASE}/approval-requests/${id}/action`, {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
  ...
  getLoyaltyConfig: async () => {
    const res = await apiFetch(`${API_BASE}/settings/loyalty`);
    if (!res.ok) throw new Error('Error al cargar config de fidelidad');
    return res.json();
  },
  updateLoyaltyConfig: async (loyalty_config: any) => {
    const res = await apiFetch(`${API_BASE}/settings/loyalty`, {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ loyalty_config })
    });
    if (!res.ok) throw new Error('Error al guardar config de fidelidad');
    return res.json();
  },
  ...
  async logLocation(lat: number, lng: number) {
    ...
    const res = await apiFetch(`${API_BASE}/settings/log-location`, {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
  ...
  async payClienteDeuda(id: string, pago: { monto: number; motivo: string; turno_id?: string; usuario_id?: string; usuario_nombre?: string }) {
    ...
    const res = await apiFetch(`${API_BASE}/clientes/${id}/pay`, {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
  ...
  async getClienteHistory(id: string) {
    ...
    const res = await apiFetch(`${API_BASE}/clientes/${id}/history`);
    return res.json();
  },
  ...
  async markNotificationsRead(notificationId?: string) {
    const res = await apiFetch(`${API_BASE}/notifications/read`, {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
  ...
  async getCommissions() {
    const res = await apiFetch(`${API_BASE}/commissions`);
    if (!res.ok) throw new Error('Error fetching commissions');
    return res.json();
  },
  async withdrawCommissions() {
    const res = await apiFetch(`${API_BASE}/commissions/withdraw`, { method: 'POST' });
    if (!res.ok) {
        const d = await res.json().catch(()=>({}));
        throw new Error(d.error || 'Error withdrawing commissions');
    }
    return res.json();
  }
};

...
import { Bell, User, Search, X, CheckCheck, Menu, Settings, Briefcase, Shield, ShieldAlert, LogOut, Store, LayoutGrid, MessageSquare, WifiOff } from 'lucide-react';
    ...
    <header className="fixed top-0 left-0 w-full md:left-[240px] md:w-[calc(100%-240px)] z-40 bg-white/80 dark:bg-vuttik-navy/80 backdrop-blur-md shadow-sm border-b border-gray-100/50 transition-all px-4 md:px-8 py-3">
      <div className="flex items-center justify-between">
        ...
        <div className={`flex items-center gap-3 md:gap-5 ml-auto relative ${showMobileSearch ? 'hidden md:flex' : 'flex'}`}>
          {!isOnline && (
            <div className="flex items-center gap-2 px-3 py-1.5 bg-red-50 text-red-600 rounded-full text-xs font-bold mr-2 border border-red-100">
              <WifiOff size={14} />
              <span className="hidden md:inline">Modo Local (Sin Conexión)</span>

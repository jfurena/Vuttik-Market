...
export default function Profile({ currentUserId, onViewProduct }: { currentUserId?: string, onViewProduct?: (id: string) => void }) {
  ...
  return (
    <div className="bg-surface text-on-surface font-body-md selection:bg-vuttik-blue selection:text-white pb-20">
      <div className="max-w-container-max mx-auto px-4 md:px-margin-desktop">
        {/* Premium Header Section */}
        <section className="relative mb-16 pt-4">
          {/* Profile Banner Background - Stitch Style Mesh Gradient */}
          <div className="w-full h-48 md:h-64 rounded-3xl overflow-hidden relative mb-[-56px] md:mb-[-72px] shadow-2xl ring-1 ring-black/5">
            <div className="absolute inset-0 bg-gradient-to-br from-indigo-600 via-purple-600 to-blue-500 opacity-90"></div>
            <div className="absolute top-0 right-0 w-96 h-96 bg-pink-500 rounded-full mix-blend-multiply filter blur-3xl opacity-50 animate-blob"></div>
            <div className="absolute top-0 left-0 w-96 h-96 bg-cyan-400 rounded-full mix-blend-multiply filter blur-3xl opacity-50 animate-blob animation-delay-2000"></div>
            <div className="absolute -bottom-8 left-20 w-96 h-96 bg-yellow-400 rounded-full mix-blend-multiply filter blur-3xl opacity-50 animate-blob animation-delay-4000"></div>
          </div>
          
          {/* User Info Overlay */}
          <div className="px-6 md:px-12 relative flex flex-col items-center md:items-start md:flex-row md:gap-8">
            <div className="relative group">
              <div className="w-36 h-36 md:w-44 md:h-44 rounded-full ring-4 ring-white shadow-2xl overflow-hidden bg-white relative z-10 transition-transform duration-300 group-hover:scale-[1.02]">
                <UserAvatar src={profileUser.photoURL || profileUser.photo_url} alt={profileUser.displayName || profileUser.display_name} />
              </div>
              <div className="absolute bottom-2 right-2 md:bottom-4 md:right-4 w-10 h-10 bg-gradient-to-r from-blue-500 to-indigo-500 rounded-full border-[3px] border-white flex items-center justify-center shadow-lg z-20">
                <ShieldCheck className="text-white" size={20} />
              </div>
            </div>
            
            <div className="mt-4 md:mt-20 text-center md:text-left flex-1 bg-white/40 backdrop-blur-md md:bg-transparent md:backdrop-blur-none p-4 rounded-2xl md:p-0">
              <div className="flex flex-col md:flex-row md:items-end md:justify-between gap-6">
                <div>
                  <h2 className="font-headline-lg text-3xl md:text-4xl text-gray-900 font-black tracking-tight mb-1">
                    {profileUser.displayName || profileUser.display_name}
                  </h2>
                  <div className="flex items-center justify-center md:justify-start gap-2 mb-3">
                    <span className="text-blue-600 font-semibold text-sm bg-blue-50 px-3 py-1 rounded-full border border-blue-100">
                      @{profileUser.username || (profileUser.email && profileUser.email.split('@')[0]) || 'usuario'}
                    </span>
                  </div>
                  <p className="font-body-md text-gray-600 text-base md:text-lg max-w-2xl leading-relaxed">
        ...
        {/* Stats Bar */}
        <section className="max-w-4xl mx-auto mb-10 relative z-20 -mt-6">
          <div className="bg-white/70 backdrop-blur-xl rounded-3xl shadow-[0_8px_30px_rgb(0,0,0,0.04)] border border-white/80 p-6 flex justify-around items-center gap-4 text-center mx-4 md:mx-0">
            <div className="flex-1 flex flex-col items-center justify-center group hover:-translate-y-1 transition-transform">
              <div className="flex items-center justify-center w-12 h-12 bg-yellow-50 rounded-2xl mb-3 text-yellow-500 shadow-inner">
                <Star size={24} className="fill-yellow-500" />
              </div>
              <span className="text-2xl text-gray-900 font-bold mb-1">
            ...
            <div className="w-px h-16 bg-gray-200/50"></div>
            <div className="flex-1 flex flex-col items-center justify-center group hover:-translate-y-1 transition-transform">
              <div className="flex items-center justify-center w-12 h-12 bg-blue-50 rounded-2xl mb-3 text-blue-500 shadow-inner">
                <Grid size={24} />
              </div>
              <span className="block text-2xl text-gray-900 font-bold mb-1">
                {userProducts.filter(p => p.price !== undefined || p.categoryId !== undefined).length}
              </span>
              <span className="text-xs text-gray-500 font-bold uppercase tracking-wider">Productos</span>
            </div>
            <div className="w-px h-16 bg-gray-200/50"></div>
            <div className="flex-1 flex flex-col items-center justify-center cursor-pointer group hover:-translate-y-1 transition-transform" onClick={() => setShowFollowersModal(true)}>
              <div className="flex items-center justify-center w-12 h-12 bg-indigo-50 rounded-2xl mb-3 text-indigo-500 shadow-inner">
                <Users size={24} />
              </div>
              <span className="block text-2xl text-gray-900 font-bold mb-1">{profileUser.followerCount || 0}</span>

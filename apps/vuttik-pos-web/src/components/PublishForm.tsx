    ...
    try {
      ...
      if (currentCategory?.fields && Array.isArray(currentCategory.fields)) {
        ...
        for (const field of currentCategory.fields) {
          if (field.required && !safeCustom[field.id]) {
            return false;
          }
        }
      }
                  ...
                  <button
                    onClick={async () => {
                      ...
                      try {
                        ...
                        alert(err.message || 'Error al enviar sugerencia.');
                      }
                    }}
          ...
          >
            ...
            {publishError && (
              <div className="w-full mt-2 px-4 py-3 rounded-2xl bg-red-50 border border-red-200 text-red-700 text-sm font-semibold flex items-start gap-2">
                ...
                <span>{typeof publishError === 'string' ? publishError : JSON.stringify(publishError)}</span>
              </div>
            )}

...
files.forEach(file => {
  if (fs.existsSync(file)) {
    ...
    // Specific typographies from DESIGN.md
    content = content.replace(/text-sm font-bold/g, 'font-body-md font-bold');
    content = content.replace(/text-xl font-black/g, 'font-headline-md font-black');
    content = content.replace(/text-xs font-black/g, 'font-label-md font-black');

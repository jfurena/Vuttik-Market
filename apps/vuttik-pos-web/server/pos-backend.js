 ...
 */
const sessionBelongsToBusiness = (s, biz) => {
    if (!biz)
        return false;
    if (s.owner_id && biz.owner_id === s.owner_id)
        return true;
    if (s.employee_id) {
        return (biz.employees || []).some((e) => e.id === s.employee_id && e.estado === 'activo');
    }
    return false;
};
...
const requireAdminBizAccess = (req, res, next) => {
    const s = req.session;
    if (!s.business_id)
    ...
    const biz = db.businesses.find((b) => b.id === s.business_id);
    if (!biz)
        return res.status(403).json({ error: 'Negocio no encontrado' });
    ...
    const emp = (biz.employees || []).find((e) => e.id === s.employee_id && e.estado === 'activo');
...
async function startServer() {
     ...
     */
    app.post('/api/market-sync/product', async (req, res) => {
        ...
        const posDb = getDB();
        const biz = posDb.businesses.find((b) => b.id === bizId);
    ...
    app.post('/api/upload', requireBizAccess, (req, res) => {
        try {
    ...
    app.use(async (req, _res, next) => {
        ...
        if (headerBizId && !req.session.business_id && req.session.owner_id) {
            const db = getDB();
            const biz = db.businesses.find((b) => b.id === headerBizId);
    ...
    app.get('/api/businesses', requireOwnerAuth, async (req, res) => {
        ...
        for (let b of db.businesses) {
            if (b.owner_id === s.owner_id) {
                try {
                    ...
                    if (profile) {
                        if (profile.name && profile.name !== b.nombre) {
                            b.nombre = profile.name;
                            dbUpdated = true;
                        }
                        if (profile.description !== undefined && profile.description !== b.description) {
                            b.description = profile.description;
                            dbUpdated = true;
                        }
                        if (profile.phone !== undefined && profile.phone !== b.phone) {
                            b.phone = profile.phone;
                            dbUpdated = true;
                        }
                        if (profile.working_hours !== undefined && profile.working_hours !== b.working_hours) {
                            b.working_hours = profile.working_hours;
                            dbUpdated = true;
                        }
                        if (profile.logo !== undefined && profile.logo !== b.logo) {
                            b.logo = profile.logo;
                            dbUpdated = true;
                        }
                        if (profile.location) {
        ...
        const myBizList = db.businesses
            ...
            const isCompraMercancia = (e) => e.es_compra_mercancia || (e.categoria && typeof e.categoria === 'string' && e.categoria.toUpperCase() === 'COMPRAS DE MERCANCÍA');
            const comprasMercancia = expenses.filter(isCompraMercancia);
            const gastosOperativos = expenses.filter((e) => e.categoria !== 'TRANSFERENCIA' && !isCompraMercancia(e));
            ...
            return {
                ...
                phone: b.phone,
                social_links: typeof b.social_links === 'string' ? (() => { try {
                    return JSON.parse(b.social_links);
                }
                catch {
                    return {};
                } })() : (b.social_links || {})
    ...
    app.post('/api/businesses', requireOwnerAuth, async (req, res) => {
        ...
        if (userBusinessesCount >= 1) {
            try {
                ...
                if (!user || !user.multi_business_approved) {
                    ...
                    await run(`INSERT INTO vuttik_business_requests (id, user_id, user_name, user_email, status, business_name, logo, location, created_at) VALUES (?, ?, ?, ?, 'pending', ?, ?, ?, ?)`, [reqId, s.owner_id, user?.display_name || '', user?.email || '', nombre || '', '', location ? JSON.stringify(location) : null, new Date().toISOString()]);
        ...
        if (description !== undefined)
            newBiz.description = description;
        if (working_hours !== undefined)
            newBiz.working_hours = working_hours;
        if (phone !== undefined)
            newBiz.phone = phone;
        if (social_links !== undefined)
            newBiz.social_links = social_links;
        db.businesses.push(newBiz);
        saveDB(db);
        const now = new Date().toISOString();
        try {
            await run(`INSERT INTO vuttik_business_profiles (uid, owner_uid, name, location, created_at, updated_at) VALUES (?, ?, ?, ?, ?, ?)`, [newBizId, s.owner_id, nombre.trim(), location ? JSON.stringify(location) : null, now, now]);
        }
        catch (err) {
    ...
    app.post('/api/pos/request-multi-business', requireOwnerAuth, async (req, res) => {
        ...
        try {
            ...
            const reqId = 'req-' + Date.now();
            await run(`INSERT INTO vuttik_business_requests (id, user_id, user_name, user_email, status, business_name, logo, location, created_at) VALUES (?, ?, ?, ?, 'pending', ?, ?, ?, ?)`, [reqId, s.owner_id, userInfo?.display_name || '', userInfo?.email || '', nombre || '', logo || '', location ? JSON.stringify(location) : null, new Date().toISOString()]);
            res.json({ success: true });
        }
        catch (err) {
    ...
    app.post('/api/businesses/claim-approved', requireOwnerAuth, async (req, res) => {
        ...
        if (!request_id)
            return res.status(400).json({ error: 'Falta request_id.' });
        try {
            const request = await get(`SELECT * FROM vuttik_business_requests WHERE id = ? AND user_id = ? AND status = 'approved'`, [request_id, s.owner_id]);
            if (!request)
                return res.status(404).json({ error: 'Solicitud aprobada no encontrada.' });
            const db = getDB();
            const existingCodes = db.businesses.map((b) => b.codigo);
            ...
            const newBiz = emptyBusiness(newBizId, nombre.trim(), codigo, s.owner_id);
            if (request.location)
                newBiz.location = JSON.parse(request.location);
            if (request.logo)
                newBiz.logo = request.logo;
            db.businesses.push(newBiz);
            saveDB(db);
            const now = new Date().toISOString();
            try {
                await run(`INSERT INTO vuttik_business_profiles (uid, owner_uid, name, location, created_at, updated_at) VALUES (?, ?, ?, ?, ?, ?)`, [newBizId, s.owner_id, nombre.trim(), request.location || null, now, now]);
            }
            catch (err) {
                console.error('Error creating business profile:', err);
            }
    ...
    app.put('/api/businesses/:bizId', requireOwnerAuth, async (req, res) => {
        ...
        db.businesses[idx].nombre = nombre.trim();
        if (logo !== undefined)
            db.businesses[idx].logo = logo;
        if (description !== undefined)
            db.businesses[idx].description = description;
        if (working_hours !== undefined)
            db.businesses[idx].working_hours = working_hours;
        if (phone !== undefined)
            db.businesses[idx].phone = phone;
        if (social_links !== undefined)
            db.businesses[idx].social_links = social_links;
        if (location !== undefined) {
            db.businesses[idx].location = location;
            ...
            const now = new Date().toISOString();
            try {
                await run(`UPDATE vuttik_business_profiles SET name = ?, location = ?, logo = ?, description = ?, working_hours = ?, phone = ?, social_links = ?, updated_at = ? WHERE uid = ?`, [nombre.trim(), JSON.stringify(location), logo || null, description || null, working_hours || null, phone || null, social_links ? JSON.stringify(social_links) : null, now, bizId]);
                await run(`UPDATE vuttik_products SET location = ?, lat = ?, lng = ?, author_name = ?, store_name = ? WHERE author_id = ?`, [address, lat, lng, nombre.trim(), nombre.trim(), bizId]);
            }
            catch (err) {
            ...
            const now = new Date().toISOString();
            try {
                await run(`UPDATE vuttik_business_profiles SET name = ?, logo = ?, description = ?, working_hours = ?, phone = ?, social_links = ?, updated_at = ? WHERE uid = ?`, [nombre.trim(), logo || null, description || null, working_hours || null, phone || null, social_links ? JSON.stringify(social_links) : null, now, bizId]);
                await run(`UPDATE vuttik_products SET author_name = ?, store_name = ? WHERE author_id = ?`, [nombre.trim(), nombre.trim(), bizId]);
            }
            catch (err) {
    ...
    app.put('/api/employees/:empId', requireOwnerBizAccess, async (req, res) => {
        ...
        if (permisos !== undefined)
            biz.employees[idx].permisos = permisos;
    ...
    app.post('/api/products', requireBizAccess, async (req, res) => {
        ...
        const locationObj = biz.location || biz.settings?.allowed_location;
        if (biz.settings && biz.settings.market_sync_url && biz.settings.market_api_key) {
            ...
            try {
                ...
                console.error('Error syncing remote:', e);
            }
        }
        else {
    ...
    app.put('/api/products/:id', requireBizAccess, async (req, res) => {
        ...
        if (biz.settings && biz.settings.market_sync_url && biz.settings.market_api_key) {
            try {
                ...
                console.error('Error syncing remote update:', e);
            }
        }
        else {
            try {
    ...
    app.delete('/api/products/:id', requireOwnerBizAccess, async (req, res) => {
        ...
        if (biz.settings && biz.settings.market_sync_url && biz.settings.market_api_key) {
            try {
                ...
                console.error('Error syncing remote delete:', e);
            }
        }
        else {
            try {
    ...
    app.post('/api/notifications/read', requireAdminBizAccess, (req, res) => {
        ...
        if (!biz.notifications)
            biz.notifications = [];
        if (notificationId) {
            const notif = biz.notifications.find((n) => n.id === notificationId);
            if (notif)
                notif.isRead = true;
        }
        else {
            biz.notifications.forEach((n) => n.isRead = true);
        }
        saveDB(db);
        res.json({ success: true });
    });
    ...
    app.get('/api/stats', requireBizAccess, (req, res) => {
        ...
        const isCompraMercancia = (e) => e.es_compra_mercancia || (e.categoria && typeof e.categoria === 'string' && e.categoria.toUpperCase() === 'COMPRAS DE MERCANCÍA');
        const gastosOperativos = filteredExpenses.filter((e) => !isCompraMercancia(e) && !e.es_transferencia && e.categoria !== 'TRANSFERENCIA');
    ...
    app.get('/api/activity-log', requireBizAccess, (req, res) => {
        ...
        if (emp?.rol === 'supervisor')
            return res.json(registro);
        return res.json(registro.filter((r) => r.usuario_id === s.employee_id));
    });
    ...
    function calculateDynamicCredit(cliente, biz) {
        let limite = cliente.limite_credito || 0;
        if (!biz.loyalty_config || !biz.loyalty_config.activa)
            return limite;
        const conf = biz.loyalty_config;
        let ventasFiltradas = (biz.sales || []).filter((s) => s.cliente_id === cliente.id && s.estado !== 'cancelada' && s.estado !== 'reembolsada');
        if (conf.mes_evaluacion !== 'historico') {
            if (conf.mes_evaluacion.includes('|')) {
                const [start, end] = conf.mes_evaluacion.split('|');
                ventasFiltradas = ventasFiltradas.filter((s) => {
        ...
        if (conf.tipo_regla === 'monto') {
            const totalGastado = ventasFiltradas.reduce((sum, s) => sum + s.total, 0);
    ...
    app.get('/api/debug-auth', requireBizAccess, (req, res) => {
        const s = req.session;
        res.json({
            owner_id: s.owner_id,
    ...
    app.get('/api/commissions', requireBizAccess, (req, res) => {
        const s = req.session;
        const db = getDB();
        const biz = getBiz(db, s.business_id);
        ...
        if (s.role !== 'admin' && s.owner_id !== userId) {
            userCommissions = biz.commissions.filter((c) => c.employee_id === userId);
        }
        else {
    ...
    app.post('/api/commissions/withdraw', requireBizAccess, (req, res) => {
        const s = req.session;
        const db = getDB();
        const biz = getBiz(db, s.business_id);
        const userId = s.employee_id || s.owner_id;
        if (!biz.commissions)
            biz.commissions = [];
        const pendingCommissions = biz.commissions.filter((c) => c.employee_id === userId && c.estado === 'pendiente');
        if (pendingCommissions.length === 0) {
            return res.status(400).json({ error: 'No hay comisiones pendientes para retirar.' });
        }
        const totalToWithdraw = pendingCommissions.reduce((sum, c) => sum + c.monto, 0);
        const activeShift = (biz.shifts || []).find((sh) => sh.estado === 'abierto' && sh.usuario_id === userId);
        if (!activeShift) {
            ...
            if (db.businesses) {
                db.businesses.forEach((biz) => {
                    if (biz.sales && Array.isArray(biz.sales)) {
                        ...
                        biz.sales = biz.sales.filter((sale) => {

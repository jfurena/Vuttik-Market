...
sqliteDb.all("SELECT * FROM vuttik_products WHERE id LIKE 'pos-%'", (err, rows) => {
  ...
  if (rows && rows.length > 0) {
    let currentDb: any = { owners: [], businesses: [] };
    ...
    if (!currentDb.businesses) currentDb.businesses = [];
    
    let recoveredCount = 0;
    for (const row of rows as any[]) {
      const biz = currentDb.businesses.find((b: any) => b.id === row.author_id);
      if (biz) {
        if (!biz.products) biz.products = [];
        
        const originalId = row.id.replace('pos-', '');
        const exists = biz.products.find((p: any) => p.id === originalId);

...
syncRouter.post('/push', async (req, res) => {
  try {
    ...
    const db = getDB();
    const biz = db.businesses.find((b: any) => b.codigo === businessCode);
    if (!biz) {
    ...
    if (sales && sales.length > 0) {
      for (const sale of sales) {
        const exists = biz.sales.find((s: any) => s.id === sale.id);
        if (!exists) {
    ...
    if (inventory_movements && inventory_movements.length > 0) {
      for (const mov of inventory_movements) {
        const exists = biz.inventory_movements.find((m: any) => m.id === mov.id);
        if (!exists) {
          const movCopy = { ...mov };
    ...
    if (products && products.length > 0) {
      ...
      for (const prod of products) {
        const idx = biz.products.findIndex((p: any) => p.id === prod.id);
        const prodCopy = { ...prod };

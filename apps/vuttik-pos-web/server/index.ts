...
app.post('/api/ai/scan-image', authenticateToken, async (req, res) => {
...
app.post('/api/pos/sync', authenticateToken, async (req: any, res) => {
  ...
  for (const op of operations) {
    try {
      ...
      if (operation === 'sale') {
        ...
        const db = getDB();
        const biz = db.businesses.find((b: any) => b.id === business_id);
        if (biz) {
          if (!biz.sales) biz.sales = [];
          // Avoid duplicates by id
          const exists = biz.sales.find((s: any) => s.id === payload.id);
          if (!exists) {
        ...
        if (biz) {
          ...
          const exists = biz.expenses.find((e: any) => e.id === payload.id);
          if (!exists) {
        ...
        if (biz) {
          ...
          const exists = biz.inventory_movements.find((m: any) => m.id === payload.id);
          if (!exists) {
            biz.inventory_movements.push({ ...payload, synced_from_offline: true });
          }
          // Update product stock
          if (payload.product_id && payload.quantity_change) {
            const prod = biz.products?.find((p: any) => p.id === payload.product_id);
            if (prod) {
...
app.get('/api/pos/businesses', authenticateToken, async (req: any, res) => {
  try {
    const db = getDB();
    const userBusinesses = db.businesses.filter((b: any) => b.owner_id === req.user.uid);
...
app.post('/api/pos/employee-login', async (req, res) => {
  ...
  if (!business_codigo || !username || !password) return res.status(400).json({ error: 'Missing fields' });
  
  try {
    const db = getDB();
    const biz = db.businesses.find((b: any) => b.codigo === business_codigo.toUpperCase().trim());
    if (!biz) return res.status(404).json({ error: 'Business code not found' });

...
authRouter.get('/mega-guardian/db/:source/:table', authenticateToken, async (req: any, res) => {
  if (req.user.role !== 'mega_guardian') return res.status(403).json({ error: 'Acceso denegado' });
  try {
...
authRouter.put('/mega-guardian/db/:source/:table/:id', authenticateToken, async (req: any, res) => {
  if (req.user.role !== 'mega_guardian') return res.status(403).json({ error: 'Acceso denegado' });
  try {
...
authRouter.delete('/mega-guardian/db/:source/:table/:id', authenticateToken, async (req: any, res) => {
  if (req.user.role !== 'mega_guardian') return res.status(403).json({ error: 'Acceso denegado' });
  try {
    const { source, table, id } = req.params;

...
async function startServer() {
  ...
  app.get('/api/businesses', requireOwnerAuth, (req, res) => {
    ...
    const myBizList = db.businesses
      ...
      .map((b: any) => {
        ...
        const comprasMercancia = expenses.filter((e: any) => e.categoria === 'Compras de Mercancía' || e.es_compra_mercancia);
        const gastosOperativos = expenses.filter((e: any) => e.categoria !== 'TRANSFERENCIA' && e.categoria !== 'Compras de Mercancía' && !e.es_compra_mercancia);
  ...
  app.get('/api/stats', requireBizAccess, (req, res) => {
    ...
    const gastosOperativos = filteredExpenses.filter((e: any) => !e.es_compra_mercancia && !e.es_transferencia);
    const comprasMercancia = filteredExpenses.filter((e: any) => e.es_compra_mercancia);
    const totalGastosOperativos = gastosOperativos.reduce((acc: number, e: any) => acc + (e.monto || 0), 0);
    const totalComprasMercancia = comprasMercancia.reduce((acc: number, e: any) => acc + (e.monto || 0), 0);

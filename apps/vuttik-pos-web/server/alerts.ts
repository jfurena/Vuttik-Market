...
export const DEFAULT_ALERT_SETTINGS: Record<AlertKind, boolean> = {
...
function esDuplicado(clave: string): boolean {
  const ahora = Date.now();
  for (const [k, t] of recientes) {
    if (ahora - t > VENTANA_ANTIRREPETICION_MS) recientes.delete(k);
  }
...
async function correoDelDueno(db: any, biz: any): Promise<string | null> {
  const propio = (biz.settings?.alert_email || '').trim();
  if (propio) return propio;

  const owner = (db.owners || []).find((o: any) => o.id === biz.owner_id);
  if (owner?.correo) return owner.correo;

  try {
    const fila: any = await get('SELECT email FROM vuttik_users WHERE uid = ?', [biz.owner_id]);
    // Las cuentas sintéticas de negocio no son buzones reales.
    if (fila?.email && !fila.email.endsWith('@business.local')) return fila.email;
  } catch {
...
const dinero = (n: number) => 'RD$' + Number(n || 0).toLocaleString('es-DO', { minimumFractionDigits: 2 });

function plantilla(titulo: string, negocio: string, filas: [string, string][], nota?: string) {
  const cuerpo = filas
...
export async function enviarAlerta(
  ...
  biz: any,
  tipo: AlertKind,
  datos: { usuario?: string; monto?: number; detalle?: string; filas?: [string, string][] }
): Promise<void> {
  try {
    const ajustes = { ...DEFAULT_ALERT_SETTINGS, ...(biz.settings?.alerts || {}) };
    if (!ajustes[tipo]) return;

    const minimo = Number(biz.settings?.alert_min_amount ?? DEFAULT_MIN_AMOUNT);
    if (typeof datos.monto === 'number' && Math.abs(datos.monto) < minimo) return;
    ...
    if (esDuplicado(clave)) return;

    const filas: [string, string][] = datos.filas || [];
    if (datos.usuario) filas.unshift(['Usuario', datos.usuario]);
    if (typeof datos.monto === 'number') filas.push(['Importe', dinero(datos.monto)]);

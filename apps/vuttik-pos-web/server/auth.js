...
const JWT_SECRET = () => {
    ...
    if (secret)
        return secret;
    // Never fall back to a known value in production: the old default was
    // published in this repository, so anyone could mint valid tokens.
    if (process.env.NODE_ENV === 'production') {
        throw new Error('JWT_SECRET no está configurado.');
    }
    return 'local-development-only-secret-not-for-production';
};
// Read from the environment. These used to be string literals split in two
// halves, which both hardcoded the credential and evaded secret scanners.
...
authRouter.post('/login', async (req, res) => {
    ...
    try {
        ...
        // Block OAuth-only users, but allow if they have a password_hash set (e.g. dev bypass)
...
authRouter.get('/me', authenticateToken, async (req, res) => {
    try {
        ...
        try {
            ...
            // Tables may not exist in dev environment, ignore
        }
        res.json({
            ...user,
            displayName,
...
authRouter.get('/business-requests', authenticateToken, async (req, res) => {
    ...
    try {
      ...
      SELECT r.id, r.user_id, r.status, r.created_at, r.business_name, r.logo, r.location,
             COALESCE(u.display_name, r.user_name) as display_name,
             COALESCE(u.email, r.user_email) as email
      FROM vuttik_business_requests r
      LEFT JOIN vuttik_users u ON r.user_id = u.uid
...
authRouter.get('/mega-guardian/db/:source/:table', authenticateToken, async (req, res) => {
    if (req.user.role !== 'mega_guardian')
        return res.status(403).json({ error: 'Acceso denegado' });
    try {
...
authRouter.put('/mega-guardian/db/:source/:table/:id', authenticateToken, async (req, res) => {
    if (req.user.role !== 'mega_guardian')
        return res.status(403).json({ error: 'Acceso denegado' });
    try {
        ...
        if (source === 'sqlite') {
            ...
            const primaryKeyCol = table === 'vuttik_users' ? 'uid' : 'id';
...
authRouter.delete('/mega-guardian/db/:source/:table/:id', authenticateToken, async (req, res) => {
    if (req.user.role !== 'mega_guardian')
        return res.status(403).json({ error: 'Acceso denegado' });
    try {
        const { source, table, id } = req.params;

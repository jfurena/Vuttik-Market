...
export const performSync = async () => {
    try {
        ...
        for (const business of db.businesses) {
            const pendingSales = business.sales.filter((s) => !s.synced);
            const pendingMovements = business.inventory_movements.filter((m) => !m.synced);
            const pendingProducts = business.products.filter((p) => !p.synced);
            ...
            try {
                ...
                if (res.ok) {
                    ...
                    pendingSales.forEach((s) => s.synced = true);
                    pendingMovements.forEach((m) => m.synced = true);
                    pendingProducts.forEach((p) => p.synced = true);
                    saveDB(db);
                }
                else {
